...
async def got_chpub_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.clear()
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("📤 Yuborish", callback_data=f"chpub_send_{code}")]])
    await update.message.reply_text(
        f"🎬 *{_esc_md(name)}*\n📡 Kanal: {channel}\n\n"
...
def main():
    ...
    channel_publish_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^📡 Anime Kanaliga Yuborish$"), channel_publish_start)],
        states={
