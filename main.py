...
ADMIN_BUTTONS = {
    ...
    "🆕 Yangi Qismlar", "🛠 Qism Boshqarish", "💾 Backup Olish", "♻️ Backup Tiklash", "🏷 Anime Holati", "🧪 Yuklama Testi",
...
_MENU_BTN_FILTER = filters.Regex(
    ...
    r"|📢 Reklama|📺 Animelar Kanali|📤 Kanalga Yuborish|👥 Adminlar|🆕 Yangi Qismlar|🛠 Qism Boshqarish|💾 Backup Olish|♻️ Backup Tiklash|🏷 Anime Holati|🧪 Yuklama Testi|⏭ Shorts.*)$"
)
...
def admin_menu_keyboard(user_id=None):
    ...
    if user_id == ADMIN_ID:
        keyboard.append([KeyboardButton("🧪 Yuklama Testi")])
...
async def handle_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle awaiting load-test request count from super admin
    if update.effective_user.id == ADMIN_ID and context.user_data.get("awaiting_load_test_n"):
        await got_load_test_n(update, context)
        return
...
# ==================== YUKLAMA TESTI ====================
# Botning bitta serverda qancha bir vaqtdagi (concurrent) so'rovni ko'tara
# olishini real o'lchash uchun. Har bir "virtual so'rov" ikkita real ishni
# bajaradi: (1) bazadan o'qish — alohida thread'da, chin konkurentlik uchun;
# (2) Telegram serveriga xavfsiz, hech kimga ko'rinmaydigan tarmoq so'rovi
# (bot.get_me()) — bu production'dagi eng og'ir qismni (tarmoq javobini)
# haqiqatan aks ettiradi. Hech kimga xabar yuborilmaydi.
LOAD_TEST_MAX_N = 1000  # bitta testda ruxsat etilgan eng ko'p so'rov soni

async def load_test_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    context.user_data["awaiting_load_test_n"] = True
    await update.message.reply_text(
        "🧪 *Yuklama testi*\n\n"
        "Nechta bir vaqtdagi (concurrent) so'rovni simulyatsiya qilay?\n"
        f"Masalan: `50`, `200`, `500` (maksimal {LOAD_TEST_MAX_N})\n\n"
        "_Har bir so'rov: bazadan real o'qish (alohida thread'da) + "
        "Telegram serveriga real, xavfsiz tarmoq so'rovi. Hech kimga "
        "xabar yuborilmaydi, faqat vaqt o'lchanadi._",
        parse_mode="Markdown"
    )

def _load_test_pick_code():
    """Testda ishlatish uchun bazadan haqiqiy (mavjud) anime kodini tanlaydi."""
    animes = get_all_animes()
    if not animes:
        return None
    return animes[0][0]  # (code, name, year, genre, total_ep) — birinchisining kodi

async def _load_test_one_request(bot, code):
    """Bitta 'virtual foydalanuvchi' so'rovini simulyatsiya qiladi.
    Ikkita real bosqichdan iborat — ikkalasi ham asl production so'roviga mos:
      1) Baza ishi alohida OS-thread'da (asyncio.to_thread) — chunki sqlite3
         chaqiruvlari sinxron, thread'siz asyncio.gather ularni chin
         konkurentlikda emas, ketma-ket bajaradi va natija noto'g'ri chiqadi.
      2) Telegramga real, lekin hech kimga xabar yubormaydigan, xavfsiz
         tarmoq so'rovi (bot.get_me()) — bu production'dagi eng og'ir qism
         (Telegram serveriga borib-kelish) ni haqiqatan aks ettiradi.
    """
    start = time.perf_counter()
    db_time = api_time = 0.0
    try:
        def _db_work():
            anime = get_anime_by_code(code)
            if anime:
    ...
    return ok, elapsed, db_time, api_time, err

async def got_load_test_n(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    if not context.user_data.get("awaiting_load_test_n"):
        return
    context.user_data["awaiting_load_test_n"] = False

    text = update.message.text.strip()
    if not text.isdigit() or int(text) <= 0:
        await update.message.reply_text("❌ Faqat musbat butun son yuboring (masalan: 100).")
        return
    n = min(int(text), LOAD_TEST_MAX_N)

    code = _load_test_pick_code()
    if code is None:
        await update.message.reply_text("❌ Testda ishlatish uchun bazada hech bo'lmasa 1 ta anime kerak.")
        return

    status_msg = await update.message.reply_text(f"⏳ {n} ta so'rov simulyatsiya qilinmoqda...")
    wall_start = time.perf_counter()
    results = await asyncio.gather(*[_load_test_one_request(context.bot, code) for _ in range(n)])
    wall_elapsed = time.perf_counter() - wall_start

    times = [r[1] for r in results]
    db_times = [r[2] for r in results]
    api_times = [r[3] for r in results]
    ...
    p95_ms = times_sorted[int(len(times_sorted) * 0.95) - 1] * 1000 if len(times_sorted) > 1 else max_ms
    avg_db_ms = (sum(db_times) / len(db_times)) * 1000
    avg_api_ms = (sum(api_times) / len(api_times)) * 1000
    throughput = n / wall_elapsed if wall_elapsed > 0 else 0

    # Sekinlashish/muzlash belgisi: eng sekin so'rov o'rtachadan necha marta uzoq ketgani
    slowdown_ratio = (max_ms / avg_ms) if avg_ms > 0 else 0
    if failures > n * 0.1:
        verdict = "🔴 Ko'p so'rov muvaffaqiyatsiz tugadi — server yoki Telegram limitiga tegib qolyapti."
    elif slowdown_ratio > 20:
        verdict = "🔴 Kuchli sekinlashish sezildi — ko'p so'rov birga kelganda server band bo'lib qolyapti."
    elif slowdown_ratio > 5:
        verdict = "🟡 Sezilarli sekinlashish bor — chegara yaqinlashyapti."
    else:
        verdict = "🟢 Sekinlashish sezilmadi — server bemalol ko'tardi."

    report = (
        f"🧪 *Yuklama testi natijasi*\n\n"
        f"📨 Yuborilgan so'rov: {n} ta\n"
        f"✅ Muvaffaqiyatli: {successes} ta\n"
        f"❌ Xato: {failures} ta\n"
        f"⏱ Jami vaqt: {wall_elapsed:.2f} soniya\n"
        f"⚡️ Tezlik: {throughput:.1f} so'rov/soniya\n\n"
        f"📊 Bitta so'rov vaqti:\n"
        f"   • O'rtacha: {avg_ms:.1f} ms\n"
        f"   • Eng tez: {min_ms:.1f} ms\n"
        f"   • Eng sekin: {max_ms:.1f} ms\n"
        f"   • 95-persentil: {p95_ms:.1f} ms\n\n"
        f"🔍 Qayerda vaqt ketyapti (o'rtacha):\n"
        f"   • Baza (SQLite): {avg_db_ms:.1f} ms\n"
        f"   • Telegram tarmoq javobi: {avg_api_ms:.1f} ms\n\n"
        f"{verdict}"
    )
    try:
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for key in ("awaiting_channel", "awaiting_channel_forward", "awaiting_channel_title",
                "awaiting_admin_id", "awaiting_load_test_n", "pending_channel", "pending_channel_link"):
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex("^🧪 Yuklama Testi$"), load_test_start))
