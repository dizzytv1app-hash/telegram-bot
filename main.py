...
async def anime_channel_episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ach1_"):
        ...
        if not episode:
            ...
            return
        seasons = get_seasons(code)
        season_label = f" — {_season_label(season[2], season[4])}" if len(seasons) > 1 else ""
        try:
            watch_url = f"https://t.me/{BOT_USERNAME}?start={code}_{season[2]}_{ep_num}"
            await context.bot.send_video(
                chat_id=chat_ref,
                video=episode[0],
                caption=f"🎬 *{_esc_md(anime[2])}*{season_label} — {ep_num}-qism",
                parse_mode="Markdown",
    ...
    if data.startswith("achall_"):
        ...
        chat_ref = int(channel_id) if channel_id.lstrip("-").isdigit() else channel_id
        anime = get_anime_by_code(code)
        name, genre, total_ep, poster_id = anime[2], anime[4], anime[5], anime[7]
        seasons = get_seasons(code)
        ...
        try:
            ...
            if poster_id:
                await context.bot.send_photo(
                    ...
                    caption=f"🎬 *{_esc_md(name)}*\n🎭 Janr: {genre}",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url_main)]])
                )
            # 2) Har bir fasl, har bir qism ketma-ket
            sent = 0
            for sid, snum, s_poster, s_total, s_ptype, s_label in seasons:
                episodes = get_episodes_list(sid)
                season_label = f" — {_season_label(snum, s_total)}" if len(seasons) > 1 else ""
                for ep_num in episodes:
                    ...
                    await context.bot.send_video(
                        chat_id=chat_ref,
                        video=episode[0],
                        caption=f"🎬 *{_esc_md(name)}*{season_label} — {ep_num}-qism",
                        parse_mode="Markdown",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url)]])
                    )
                    sent += 1
            ...
            await query.message.reply_text(f"✅ Yuborish tugadi! Jami {sent} ta qism kanalga joylandi.")
        except Exception as e:
            await query.message.reply_text(f"❌ Yuborishda xatolik: {e}")
        return
