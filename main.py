...
def update_season_total_episodes(season_id, total_episodes):
    conn = sqlite3.connect("anime.db")
    c = conn.cursor()
    c.execute("UPDATE seasons SET total_episodes=? WHERE id=?", (total_episodes, season_id))
    conn.commit()
    conn.close()

def update_season_poster(season_id, poster_file_id):
    conn = sqlite3.connect("anime.db")
    c = conn.cursor()
    c.execute("UPDATE seasons SET poster_file_id=? WHERE id=?", (poster_file_id, season_id))
    conn.commit()
    conn.close()
...
async def admin_new_episode_channel_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["awaiting_episode_poster"] = {
        ...
        "total_ep": total_ep, "name": name,
    }
    await query.message.reply_text(
        f"🖼 *{_esc_md(name)}* — {season_num}-fasl uchun kanalga yuboriladigan rasmni (poster) yuboring:",
        parse_mode="Markdown"
    )
...
async def add_episode_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for code, name, year, genre, total_ep in animes:
        ...
        if not seasons:
            ...
            continue
        for sid, snum, poster_id, s_total in seasons:
            added = len(get_episodes_list(sid))
            if added < s_total:
                incomplete.append((code, name))
                break
...
async def got_edit_field_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "editfield_episodes":
        ...
        if len(seasons) == 1:
            ...
            return WAIT_EDIT_VALUE
        buttons = [
            [InlineKeyboardButton(f"{snum}-fasl (hozir: {total_ep} ta)", callback_data=f"editseason_{sid}")]
            for sid, snum, poster_id, total_ep in seasons
        ]
        buttons.append([InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")])
        await query.message.reply_text(
...
async def got_edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "season_episodes":
        ...
        update_season_total_episodes(season_id, value)
        context.user_data.clear()
        await update.message.reply_text(
            f"✅ *Muvaffaqiyatli yangilandi!*\n\nKod: *{code}* — Jami qismlar → `{value}`",
            parse_mode="Markdown",
            reply_markup=admin_menu_keyboard(update.effective_user.id)
        )
        return ConversationHandler.END
...
async def show_anime_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for a in animes:
        ...
        for sid, snum, poster_id, s_total in seasons:
            added = len(get_episodes_list(sid))
            parts.append(f"{snum}-fasl: {added}/{s_total}")
