...
async def got_episode_channel_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name = info["name"]
    anime = get_anime_by_code(code)
    target_channel = get_anime_channel(code) or CHANNEL_USERNAME
    watch_url = f"https://t.me/{BOT_USERNAME}?start={code}"
...
async def channel_publish_send_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channel:
        ...
        return
    await query.answer("⏳ Yuborilmoqda, biroz vaqt olishi mumkin...")
    ...
    failed_count = 0
    for sid, snum, s_poster, s_total, s_ptype, s_label in seasons:
        episode_nums = get_episodes_list(sid)
        for ep_num in episode_nums:
            caption = f"{name}\n{name} [{ep_num:02d}-qism]\n{channel}"
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url)]])
            try:
                if s_ptype == "video":
                    await context.bot.send_video(chat_id=channel, video=s_poster, caption=caption, reply_markup=kb)
                else:
                    await context.bot.send_photo(chat_id=channel, photo=s_poster, caption=caption, reply_markup=kb)
                sent_count += 1
            except Exception as e:
                logger.warning(f"Qism postini yuborishda xato ({code}, {ep_num}-qism): {e}")
                failed_count += 1
            await asyncio.sleep(1.2)

    summary = (
        f"✅ *{_esc_md(name)}* to'liq {_esc_md(channel)} kanaliga joylandi!\n\n"
        f"📺 Yuborilgan qismlar: {sent_count}"
    )
    if failed_count:
        summary += f"\n⚠️ Yuborilmagan qismlar: {failed_count}"
...
async def got_channel_send_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_channel = get_anime_channel(code)
    seasons = get_seasons(code)
    if not seasons:
