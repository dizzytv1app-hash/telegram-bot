...
async def got_start_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.video:
        await update.message.reply_text("❗️ Iltimos, video yuboring (rasm yoki fayl emas).")
        return WAIT_START_VIDEO
    file_id = update.message.video.file_id
    set_setting("start_video_file_id", file_id)
    await update.message.reply_text("✅ Video saqlandi! Endi /start bosilganda shu video chiqadi.")
    return ConversationHandler.END
...
def main():
    ...
    set_start_video_conv = ConversationHandler(
        ...
        states={
            WAIT_START_VIDEO: [MessageHandler(filters.VIDEO, got_start_video)],
        },
