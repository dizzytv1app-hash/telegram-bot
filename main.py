...
(
    ...
    WAIT_ANIME_ORIGINAL, WAIT_ANIME_RATING,
    WAIT_ANALYZE_CODE, WAIT_ANALYZE_TEXT, WAIT_ANALYZE_MISSING,
    WAIT_POSTER_CODE, WAIT_POSTER_CHOICE, WAIT_POSTER_MEDIA,
) = range(31)
...
ADMIN_BUTTONS = {
    ...
    "🧠 Anime To'liq Tahlil", "🖼 Poster Almashtirish"
}
...
_MENU_BTN_FILTER = filters.Regex(
    ...
    r"|📢 Reklama|📺 Animelar Kanali|📤 Kanalga Yuborish|👥 Adminlar|🆕 Yangi Qismlar|🛠 Qism Boshqarish|💾 Backup Olish|♻️ Backup Tiklash|🏷 Anime Holati|🧠 Anime To'liq Tahlil|🖼 Poster Almashtirish|⏭ Shorts.*)$"
)
...
def update_anime_poster(code, poster_file_id, poster_type="photo"):
    """Tanlangan animening asosiy posterini yangilaydi."""
    conn = get_db()
    c = conn.cursor()
    c.execute(
        "UPDATE animes SET poster_file_id=?, poster_type=? WHERE code=?",
        (poster_file_id, poster_type, code),
    )
    conn.commit()


def update_episode_posters_for_anime(code, poster_file_id, poster_type="photo"):
    """Tanlangan animening barcha fasllaridagi qism posterlarini birdan yangilaydi."""
    conn = get_db()
    c = conn.cursor()
    c.execute(
        "UPDATE seasons SET poster_file_id=?, poster_type=? WHERE anime_code=?",
        (poster_file_id, poster_type, code),
    )
    conn.commit()


def update_anime_episode_meta(code, total_episodes, total_episodes_label=None):
    """Anime qism sonini yangilaydi; bitta faslli animeda 1-faslni ham sinxronlaydi."""
    conn = get_db()
    c = conn.cursor()
    c.execute(
        "UPDATE animes SET total_episodes=?, total_episodes_label=? WHERE code=?",
        (total_episodes, total_episodes_label, code),
    )
    c.execute("SELECT id FROM seasons WHERE anime_code=? ORDER BY season_num", (code,))
    seasons = c.fetchall()
    if len(seasons) == 1:
        c.execute(
            "UPDATE seasons SET total_episodes=?, total_episodes_label=? WHERE id=?",
            (total_episodes, total_episodes_label, seasons[0][0]),
        )
    conn.commit()
...
def admin_menu_keyboard(user_id=None):
    keyboard = [
        ...
        [KeyboardButton("🧠 Anime To'liq Tahlil"), KeyboardButton("🖼 Poster Almashtirish")],
    ]
    if user_id == ADMIN_ID:
...
def episodes_keyboard(season_id, page=0, show_bulk=False):
    ...
    if show_bulk and episodes:
        buttons.append([
            InlineKeyboardButton(f"📥 Barchasini yuklash ({len(episodes)} ta)", callback_data=f"bulkdl_{season_id}")
        ])
...
async def _send_season_info(bot, chat_id, name, genre, desc, season_id, poster_id, total_ep, code=None, season_num=None, show_season_label=False, poster_type="photo", ep_label=None, status_key=None):
    ...
    if not poster_id:
        await bot.send_message(
            ...
            reply_markup=episodes_keyboard(season_id, show_bulk=is_admin(chat_id)) if episodes else None
        )
        return
...
async def send_anime_info(bot, chat_id, code):
    ...
    main_poster_type = anime[11] if len(anime) > 11 and anime[11] else "photo"
    status_key = get_anime_status(code)
    seasons = get_seasons(code)

    if not seasons:
        # eski/fasl yaratilmagan holat uchun zaxira yo'l
        await _send_season_info(bot, chat_id, name, genre, desc, None, poster_id, total_ep, code=code, poster_type=main_poster_type, status_key=status_key)
        return
    ...
    if len(seasons) == 1:
        ...
        await _send_season_info(
            ...
            code=code, season_num=snum, show_season_label=False, poster_type=s_ptype, ep_label=s_label,
...
async def show_season_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _send_season_info(
        ...
        season_id, poster_id, total_ep, code=anime_code, season_num=season_num, show_season_label=True,
...
async def post_anime_to_channel(context, code, name, genre, total_episodes, poster_id, poster_type="photo", ep_label=None):
    ...
    anime_record = get_anime_by_code(code)
    if anime_record and anime_record[7]:
        poster_id = anime_record[7]
        poster_type = anime_record[11] if len(anime_record) > 11 and anime_record[11] else "photo"
...
async def _episode_send_kwargs(bot, chat_id, video_file_id, caption, season):
    """Saqlangan video file_id bilan yuborilganda qism coverini haqiqiy thumbnail sifatida
    qayta yuklab beradi. Telegram thumbnail uchun file_id emas, yangi upload talab qilishi mumkin."""
    kwargs = {
        "chat_id": chat_id,
        "video": video_file_id,
        "caption": caption,
    }
    if season and season[3] and season[5] == "photo":
        try:
            cover_file = await bot.get_file(season[3])
            cover_bytes = await cover_file.download_as_bytearray()
            # Telegram video thumbnaili JPEG va kichik hajmda bo‘lishi kerak.
            # Admin yuborgan katta posterlarni shu yerda mos o‘lcham/hajmga keltiramiz.
            image = Image.open(BytesIO(cover_bytes)).convert("RGB")
            image.thumbnail((320, 320), Image.Resampling.LANCZOS)
            encoded = BytesIO()
            quality = 82
            image.save(encoded, format="JPEG", quality=quality, optimize=True)
            while encoded.tell() > 190_000 and quality > 35:
                quality -= 8
                encoded = BytesIO()
                image.save(encoded, format="JPEG", quality=quality, optimize=True)
            encoded.seek(0)
            kwargs["thumbnail"] = InputFile(encoded, filename="episode_cover.jpg")
        except Exception as exc:
            logger.warning("Qism coverini thumbnail sifatida tayyorlab bo‘lmadi: %s", exc)
    return kwargs
...
async def episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("page_"):
        ...
        await query.edit_message_reply_markup(
            reply_markup=episodes_keyboard(int(season_id), int(page), show_bulk=is_admin(query.from_user.id))
        )
        return

    if data.startswith("bulkdl_"):
        season_id = int(data.split("_", 1)[1])
        user = query.from_user
        if not is_admin(user.id):
        ...
        episodes = get_episodes_list(season_id)
        if not episodes:
            await query.answer("❌ Bu faslda hali qism yo'q.", show_alert=True)
            return
        season = get_season_by_id(season_id)
        anime_name = ""
        if season:
            ...
            if anime:
                anime_name = anime[2]
        await query.message.reply_text(f"📥 {len(episodes)} ta qism yuborilmoqda, biroz kuting...")
        sent, failed = 0, 0
        for ep_num in episodes:
            ...
            if not episode:
                ...
                continue
            try:
                send_kwargs = await _episode_send_kwargs(
                    context.bot, user.id, episode[0],
                    f"🎬 {anime_name} — {ep_num}-qism", season
                )
                ...
                failed += 1
                logger.warning(
                    "Ommaviy yuklashda qism yuborilmadi (season=%s, episode=%s): %s",
                    season_id, ep_num, e,
                )
            await asyncio.sleep(0.7)  # Telegram flood-limitiga tushib qolmaslik uchun
        summary = f"✅ {sent} ta qism yuborildi."
        if failed:
            summary += f"\n⚠️ {failed} ta qism yuborilmadi (fayl topilmadi)."
    ...
    if data.startswith("ep_"):
        ...
        if episode:
            ...
            try:
                send_kwargs = await _episode_send_kwargs(
                    context.bot, query.from_user.id, episode[0],
                    f"🎬 {anime_name} — {ep_num}-qism", season
                )
                await context.bot.send_video(**send_kwargs)
            except Exception as e:
...
async def recent_episodes_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    seen_codes = []
    for anime_code, ep_num, added_at, name, season_num in recent:
        label = f"{name} — {season_num}-fasl — {ep_num}-qism"
...
# -- TO'LIQ ANIME TAHLILI --
ANALYZE_FIELD_ORDER = [
    "name", "original_title", "year", "genre", "description", "episodes", "rating", "status"
]
ANALYZE_FIELD_LABELS = {
    "name": "Anime nomi",
    "original_title": "Original nomi",
    "year": "Chiqarilgan yili",
    "genre": "Janr",
    "description": "Qisqa syujet",
    "episodes": "Necha qism",
    "rating": "Reyting",
    "status": "Holati",
}
ANALYZE_PROMPTS = {
    ...
    "original_title": "🔤 Original nomini kiriting:",
    "year": "📅 Chiqarilgan yilini kiriting (4 xonali raqam):",
    "genre": "🎭 Janrini kiriting:",
    "description": "📝 Qisqa syujetini kiriting:",
    "episodes": "📺 Necha qism ekanini kiriting (masalan: 24 yoki 24 qism):",
    "rating": "⭐ Reytingni kiriting (0–10):",
    "status": "🏷 Holatini kiriting: Davom etmoqda, Tugagan, Tez orada yoki Tanaffusda:",
}


def _analysis_clean(value):
    if value is None:
        return None
...
def _find_rating(text):
    match = re.search(r"(?i)(?:reyting|rating|оценка|imdb)\s*[:\-]?\s*(10(?:[.,]0)?|[0-9](?:[.,][0-9])?)", text)
    if not match:
        return None
    try:
...
def _find_status(text):
    low = text.lower()
    synonyms = [
        ("ongoing", "ongoing"), ("davom etmoqda", "ongoing"), ("ongoing", "ongoing"),
        ("finished", "finished"), ("tugagan", "finished"), ("завершен", "finished"),
        ("soon", "soon"), ("tez orada", "soon"), ("скоро", "soon"),
        ("paused", "paused"), ("tanaffusda", "paused"), ("пауза", "paused"),
    ]
    for phrase, key in synonyms:
        if phrase in low:
            return key
    return None


def _parse_anime_text(raw):
    """Labeled yoki oddiy ko‘p qatorli anime matnidan maydonlarni ajratadi."""
    lines = [line.strip() for line in raw.splitlines() if line.strip()]
    text = "\n".join(lines)
    data = {}
    data["name"] = _line_value(lines, ["anime nomi", "nomi", "title", "name", "название"])
    data["original_title"] = _line_value(lines, ["original nomi", "original title", "original", "yaponcha nomi", "оригинальное название"])
    year_value = _line_value(lines, ["chiqarilgan yili", "chiqarilgan yil", "yili", "year", "год"])
    year = _find_year(year_value or text)
    data["year"] = year
    data["genre"] = _line_value(lines, ["janr", "genre", "жанр"])
    if not data["genre"]:
        found_genres = [genre for genre in GENRE_LIST if genre.lower() in text.lower()]
        if found_genres:
            data["genre"] = ", ".join(found_genres)
    data["description"] = _line_value(lines, ["qisqa syujet", "syujet", "tavsif", "description", "сюжет", "описание"])
    episodes_value = _line_value(lines, ["necha qism", "qism soni", "qismlar", "episodes", "episode", "серии"])
    episode_match = re.search(r"(?i)(?:necha\s*qism|qism\s*soni|qismlar|episodes?|серии?)\s*[:\-]?\s*([0-9]+(?:\s*[^\n]*)?)", text)
    if episodes_value:
        parsed_episodes = _parse_episode_count(episodes_value)
        data["episodes"] = parsed_episodes if parsed_episodes[0] > 0 else None
    elif episode_match:
        parsed_episodes = _parse_episode_count(episode_match.group(1))
        data["episodes"] = parsed_episodes if parsed_episodes[0] > 0 else None
    else:
        data["episodes"] = None
    data["rating"] = _find_rating(text)
    status = _line_value(lines, ["holati", "status", "статус"])
    data["status"] = _find_status(status or text)

    # Erkin matn uchun birinchi mazmunli qator nom sifatida ishlatiladi.
    if not data["name"] and lines:
        first = lines[0]
        if not re.match(r"(?i)^(original|yili|year|janr|genre|syujet|tavsif|qism|episodes?|reyting|rating|holati|status)\b", first):
            data["name"] = first
    # Labeled description bo‘lmasa, mazmunli uzun qatorni syujet deb olamiz.
    if not data["description"]:
        for line in lines[1:]:
            low = line.lower()
            if len(line) >= 35 and not any(key in low for key in ("janr", "genre", "qism", "episode", "reyting", "rating", "holat", "status", "yil", "year")):
                data["description"] = line
                break
    return data


def _analysis_display_value(field, value):
    if field == "status":
        return ANIME_STATUSES.get(value, "—")
    if field == "episodes" and value:
        return _episode_label(value[0], value[1])
    if value is None or value == "":
        return "—"
    return str(value)


def _analysis_missing_fields(context):
    data = context.user_data.get("analysis_data", {})
    answered = context.user_data.get("analysis_answered", set())
    return [field for field in ANALYZE_FIELD_ORDER if field not in answered and data.get(field) in (None, "")]


def _analysis_preview_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Saqlash", callback_data="analysis_save")],
        [InlineKeyboardButton("❌ Bekor qilish", callback_data="analysis_cancel")],
    ])


async def _show_analysis_preview(message, context):
    data = context.user_data.get("analysis_data", {})
    code = context.user_data.get("analysis_code")
    rows = [
        f"1️⃣ Kod: {code}",
        f"2️⃣ Anime nomi: {_esc_md(_analysis_display_value('name', data.get('name')))}",
        f"3️⃣ Original nomi: {_esc_md(_analysis_display_value('original_title', data.get('original_title')))}",
        f"4️⃣ Chiqarilgan yili: {_analysis_display_value('year', data.get('year'))}",
        f"5️⃣ Janr: {_esc_md(_analysis_display_value('genre', data.get('genre')))}",
        f"6️⃣ Qisqa syujet: {_esc_md(_analysis_display_value('description', data.get('description')))}",
        f"7️⃣ Necha qism: {_esc_md(_analysis_display_value('episodes', data.get('episodes')))}",
        "8️⃣ Poster: o‘zgarmaydi",
        f"9️⃣ Reyting: {_analysis_display_value('rating', data.get('rating'))}",
        f"🔟 Holati: {_esc_md(_analysis_display_value('status', data.get('status')))}",
    ]
    await message.reply_text(
        "🔎 *Tekshirish — saqlashdan oldin to‘liq ma’lumotlar:*\n\n" + "\n".join(rows) +
        "\n\nMa’lumotlar to‘g‘ri bo‘lsa, saqlashni tasdiqlang.",
        parse_mode="Markdown",
        reply_markup=_analysis_preview_keyboard(),
    )


async def _ask_next_analysis_field(message, context):
    missing = _analysis_missing_fields(context)
    if not missing:
        await _show_analysis_preview(message, context)
        return WAIT_ANALYZE_MISSING
    field = missing[0]
    context.user_data["analysis_missing_field"] = field
    await message.reply_text(ANALYZE_PROMPTS[field], reply_markup=_cancel_kb())
    return WAIT_ANALYZE_MISSING


async def analyze_anime_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    context.user_data.clear()
    await update.message.reply_text(
        "🧠 *Anime ma’lumotlarini to‘liq tahlillash*\n\nAnime kodini kiriting:",
        parse_mode="Markdown",
        reply_markup=ReplyKeyboardRemove(),
    )
    await update.message.reply_text("👇", reply_markup=_cancel_kb())
    return WAIT_ANALYZE_CODE


async def got_analyze_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = await _check_menu_escape(update, context)
    if result is not None:
        return result
    text = update.message.text.strip()
    if not text.isdigit():
        await update.message.reply_text("⚠️ Anime kodini faqat raqam ko‘rinishida yuboring:")
        return WAIT_ANALYZE_CODE
    code = int(text)
    anime = get_anime_by_code(code)
    if not anime:
        await update.message.reply_text("❌ Bunday anime topilmadi. Kodni qayta kiriting:")
        return WAIT_ANALYZE_CODE
    context.user_data["analysis_code"] = code
    await update.message.reply_text(
        f"✅ *{_esc_md(anime[2])}* topildi.\n\nAnime haqidagi xom matnni yuboring. Poster bu jarayonda o‘zgarmaydi:",
        parse_mode="Markdown",
        reply_markup=_cancel_kb(),
    )
    return WAIT_ANALYZE_TEXT


async def got_analyze_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = await _check_menu_escape(update, context)
    if result is not None:
        return result
    raw = update.message.text.strip()
    if not raw:
        await update.message.reply_text("⚠️ Xom matn bo‘sh bo‘lmasin. Qaytadan yuboring:")
        return WAIT_ANALYZE_TEXT
    parsed = _parse_anime_text(raw)
    context.user_data["analysis_data"] = parsed
    context.user_data["analysis_answered"] = {
        field for field in ANALYZE_FIELD_ORDER
        if parsed.get(field) not in (None, "")
    }
    return await _ask_next_analysis_field(update.message, context)


async def got_analyze_missing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = await _check_menu_escape(update, context)
    if result is not None:
        return result
    field = context.user_data.get("analysis_missing_field")
    if not field:
        return await _ask_next_analysis_field(update.message, context)
    raw = update.message.text.strip()
    if not raw:
        await update.message.reply_text(ANALYZE_PROMPTS[field])
        return WAIT_ANALYZE_MISSING
    ...
    if field == "year":
        if value is None:
            ...
            await update.message.reply_text("⚠️ 4 xonali yil kiriting:")
            return WAIT_ANALYZE_MISSING
        value = parsed
    elif field == "episodes":
        if value is None:
            ...
            parsed = _parse_episode_count(value)
            if parsed[0] <= 0:
                await update.message.reply_text("⚠️ Qism sonini raqam bilan kiriting:")
                return WAIT_ANALYZE_MISSING
        value = parsed
    elif field == "rating":
        if value is None:
            ...
            try:
                ...
                await update.message.reply_text("⚠️ Reyting 0 dan 10 gacha bo‘lishi kerak:")
                return WAIT_ANALYZE_MISSING
    elif field == "status":
        value = _find_status(value or "")
        if not value:
            await update.message.reply_text(ANALYZE_PROMPTS[field])
            return WAIT_ANALYZE_MISSING
    else:
        value = _analysis_clean(value)
    context.user_data.setdefault("analysis_data", {})[field] = value
    context.user_data.setdefault("analysis_answered", set()).add(field)
    return await _ask_next_analysis_field(update.message, context)


async def analysis_save_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "analysis_cancel":
        context.user_data.clear()
        await query.message.edit_text("❌ Tahlilni saqlash bekor qilindi.")
        await query.message.reply_text("👑 Admin paneli:", reply_markup=admin_menu_keyboard(query.from_user.id))
        return ConversationHandler.END
    code = context.user_data.get("analysis_code")
    data = context.user_data.get("analysis_data", {})
    missing = _analysis_missing_fields(context)
    if missing:
        await query.answer("Avval barcha maydonlarni to‘ldiring.", show_alert=True)
        return WAIT_ANALYZE_MISSING
    try:
        update_anime_field(code, "name", data["name"] or "")
        update_anime_field(code, "original_title", data["original_title"] or None)
        update_anime_field(code, "year", data["year"] or 0)
        update_anime_field(code, "genre", data["genre"] or "")
        update_anime_field(code, "description", data["description"] or "")
        episodes, label = data["episodes"]
        update_anime_episode_meta(code, episodes, label)
        update_anime_field(code, "rating", data["rating"])
        set_anime_status(code, data["status"])
        await query.message.edit_text("✅ Anime ma’lumotlari muvaffaqiyatli yangilandi. Poster o‘zgarmadi.")
        await query.message.reply_text("👑 Admin paneli:", reply_markup=admin_menu_keyboard(query.from_user.id))
    except Exception as exc:
        logger.exception("Anime tahlil ma’lumotlarini saqlashda xato")
        await query.message.reply_text(f"❌ Saqlashda xato: {exc}", reply_markup=admin_menu_keyboard(query.from_user.id))
    context.user_data.clear()
    return ConversationHandler.END


# -- POSTER ALMASHTIRISH --
async def poster_change_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    context.user_data.clear()
    await update.message.reply_text(
    ...
    await update.message.reply_text("👇", reply_markup=_cancel_kb())
    return WAIT_POSTER_CODE


async def got_poster_change_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = await _check_menu_escape(update, context)
    if result is not None:
        return result
    raw = update.message.text.strip()
    ...
    await update.message.reply_text(
        f"✅ *{_esc_md(anime[2])}* topildi. Qaysi poster almashtiriladi?",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🖼 Asosiy anime posteri", callback_data="poster_type_main")],
            [InlineKeyboardButton("🎞 Qism/epizod posteri", callback_data="poster_type_episode")],
            [InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")],
        ]),
    )
    return WAIT_POSTER_CHOICE


async def poster_type_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "cancel_anime":
        context.user_data.clear()
        await query.message.reply_text("❌ Bekor qilindi.", reply_markup=admin_menu_keyboard(query.from_user.id))
        return ConversationHandler.END
    mode = "main" if query.data == "poster_type_main" else "episode"
    context.user_data["poster_mode"] = mode
    label = "asosiy anime posteri" if mode == "main" else "barcha fasl va qismlarga qo‘yiladigan qism posteri"
    await query.message.edit_text(f"🖼 Endi {label} uchun rasm yoki video yuboring:")
    return WAIT_POSTER_MEDIA


async def got_poster_change_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.photo and not update.message.video:
        await update.message.reply_text("⚠️ Faqat rasm yoki video yuboring:")
        return WAIT_POSTER_MEDIA
    if update.message.video:
    ...
    mode = context.user_data.get("poster_mode")
    try:
        if mode == "main":
            update_anime_poster(code, poster_id, poster_type)
            message = "✅ Asosiy anime posteri almashtirildi."
        else:
            update_episode_posters_for_anime(code, poster_id, poster_type)
            message = "✅ Qism posteri barcha fasllar va mavjud qismlarga qo‘llandi. Keyingi qismlar ham shu poster bilan ishlaydi."
        context.user_data.clear()
        await update.message.reply_text(message, reply_markup=admin_menu_keyboard(update.effective_user.id))
    except Exception as exc:
        logger.exception("Poster almashtirishda xato")
        await update.message.reply_text(f"❌ Poster saqlashda xato: {exc}", reply_markup=admin_menu_keyboard(update.effective_user.id))
        context.user_data.clear()
    return ConversationHandler.END
...
async def _interrupt_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    direct_map = {
        ...
        "🧠 Anime To'liq Tahlil": analyze_anime_start,
        "🖼 Poster Almashtirish": poster_change_start,
    }
...
def main():
    ...
    # Full anime analysis conversation
    analyze_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🧠 Anime To\'liq Tahlil$"), analyze_anime_start)],
        states={
            WAIT_ANALYZE_CODE: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_analyze_code)],
            WAIT_ANALYZE_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_analyze_text)],
            WAIT_ANALYZE_MISSING: [
                CallbackQueryHandler(analysis_save_callback, pattern="^analysis_(save|cancel)$"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, got_analyze_missing),
            ],
        },
        fallbacks=_esc + [CallbackQueryHandler(cancel_anime_callback, pattern="^cancel_anime$")],
        per_message=False,
    ...
    # Poster replacement conversation
    poster_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🖼 Poster Almashtirish$"), poster_change_start)],
        states={
            WAIT_POSTER_CODE: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_poster_change_code)],
            WAIT_POSTER_CHOICE: [CallbackQueryHandler(poster_type_callback, pattern="^(poster_type_(main|episode)|cancel_anime)$")],
            WAIT_POSTER_MEDIA: [MessageHandler(filters.PHOTO | filters.VIDEO, got_poster_change_media)],
        },
        fallbacks=_esc + [CallbackQueryHandler(cancel_anime_callback, pattern="^cancel_anime$")],
        per_message=False,
    )
