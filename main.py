...
# ==================== CONFIG ====================
BOT_TOKEN = "8639314799:AAEKj_17gjkyGCYWLM-FMbSZifjyUS_Defo"
ADMIN_ID = 6222096713
ADMIN_USERNAME = "@Reyimberganov_i"
CHANNEL_USERNAME = "@animelarr7"
CHANNEL_LINK = "https://t.me/animelarr7"
...
async def restore_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tmp_path = "/tmp/restore_upload.db"
    try:
        tg_file = await context.bot.get_file(file_id)
        await tg_file.download_to_drive(tmp_path)

        # Yuborilgan fayl haqiqatan bizning tuzilishga mos sqlite baza ekanini tekshiramiz
        test_conn = sqlite3.connect(tmp_path)
        test_conn.execute("SELECT COUNT(*) FROM animes")
        test_conn.close()
    except Exception as e:
        if os.path.exists(tmp_path):
            os.remove(tmp_path)
        await query.message.edit_text(f"❌ Fayl noto'g'ri yoki buzilgan, tiklanmadi:\n{e}")
        return
    ...
    try:
        ...
        # Eski -wal / -shm fayllarini tozalaymiz, aks holda ular eski
        # ma'lumotni yangi tiklangan fayl ustidan qayta yozib qo'yishi mumkin
        for suffix in ("-wal", "-shm"):
            stale_path = "anime.db" + suffix
            if os.path.exists(stale_path):
                os.remove(stale_path)

        shutil.copy(tmp_path, "anime.db")
        os.remove(tmp_path)
        get_db()  # yangi faylga qayta ulanadi

        await query.message.edit_text("✅ Backup muvaffaqiyatli tiklandi.")
    except Exception as e:
        logging.exception("Backup tiklashda xatolik")
        await query.message.edit_text(f"❌ Tiklashda xatolik yuz berdi:\n{e}")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(episode_callback, pattern="^(noop|page_|ep_)"))
