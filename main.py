...
_MENU_BTN_FILTER = filters.Regex(
    ...
    r"|📢 Reklama|📺 Animelar Kanali|📤 Kanalga Yuborish|👥 Adminlar|⏭ Shorts.*)$"
)
...
def add_episode(anime_code, episode_num, video_file_id):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT OR REPLACE INTO episodes (anime_code, episode_num, video_file_id)
        VALUES (?, ?, ?)
    """, (anime_code, episode_num, video_file_id))
    conn.commit()
    conn.close()
...
def genre_select_keyboard(selected):
    ...
    if row:
        buttons.append(row)
    buttons.append([InlineKeyboardButton(f"➡️ Tasdiqlash ({len(selected)}/{GENRE_MAX_SELECT})", callback_data="gconfirm")])
...
async def send_anime_info(bot, chat_id, code):
    ...
    if not anime:
        ...
        return
    _, code, name, year, genre, total_ep, desc, poster_id, added_date = anime
    ...
    if desc:
        caption = caption.replace("👇 Qismni tanlang:", f"📝 {desc}\n\n👇 Qismni tanlang:")
...
async def got_anime_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"3️⃣ Janrlarni tanlang _({GENRE_MIN_SELECT}-{GENRE_MAX_SELECT} ta)_:",
        parse_mode="Markdown",
...
async def got_anime_genre_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("gsel_"):
        ...
        if idx in selected:
            ...
            await query.answer()
        elif len(selected) >= GENRE_MAX_SELECT:
            await query.answer(f"⚠️ Ko'pi bilan {GENRE_MAX_SELECT} ta janr tanlang!", show_alert=True)
            return WAIT_ANIME_GENRE
        else:
            selected.add(idx)
            await query.answer()
    ...
    if data == "gconfirm":
        if len(selected) < GENRE_MIN_SELECT:
            await query.answer(f"⚠️ Kamida {GENRE_MIN_SELECT} ta janr tanlang!", show_alert=True)
            return WAIT_ANIME_GENRE
        await query.answer()
...
# -- ADD EPISODE --
def _done_ep_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ Tayyor (yakunlash)", callback_data="done_episodes")]])
...
async def add_episode_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not animes:
        ...
        return ConversationHandler.END
    context.user_data.clear()
    lines = "\n".join(f"*{a[0]}* — {a[1]} ({a[2]})" for a in animes)
    await update.message.reply_text(
...
async def got_episode_anime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"2️⃣ Qism raqamini yuboring _(tavsiya: {next_num})_:",
        parse_mode="Markdown",
        reply_markup=_done_ep_kb()
    )
    return WAIT_EPISODE_NUM

async def got_episode_num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    if not txt.isdigit() or int(txt) <= 0:
        await update.message.reply_text(
        ...
        return WAIT_EPISODE_NUM
    context.user_data["ep_num"] = int(txt)
    await update.message.reply_text(
        f"3️⃣ *{int(txt)}-qism* videosini yuboring:",
        parse_mode="Markdown",
        reply_markup=_done_ep_kb()
    )
...
async def got_episode_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = context.user_data
    add_episode(d["ep_anime_code"], d["ep_num"], file_id)
    d["ep_added_count"] = d.get("ep_added_count", 0) + 1
    next_num = d["ep_num"] + 1
    await update.message.reply_text(
        f"✅ *{d['ep_anime_name']} — {d['ep_num']}-qism* saqlandi!\n\n"
        f"Keyingi qism raqamini yuboring _(tavsiya: {next_num})_\n"
        f"yoki yakunlash uchun tugmani bosing:",
        parse_mode="Markdown",
        reply_markup=_done_ep_kb()
    )
    return WAIT_EPISODE_NUM
...
async def got_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    delete_anime(code)
    context.user_data.clear()
    await query.message.edit_text(f"✅ *{anime[2]}* o'chirildi!", parse_mode="Markdown")
    await query.message.reply_text("👑 Admin paneli:", reply_markup=admin_menu_keyboard(update.effective_user.id))
    return ConversationHandler.END
...
async def show_anime_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for a in animes:
        ...
        text += f"*{a[0]}* — {a[1]} ({a[2]}) | {len(episodes)}/{a[4]} qism\n"
...
async def chsend_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        await query.message.reply_text(f"✅ *{name}* kanalga yuborildi!", parse_mode="Markdown")
    else:
        await query.message.reply_text(f"❌ *{name}* kanalga yuborilmadi. Bot kanalda admin ekanligini tekshiring.", parse_mode="Markdown")
...
def main():
    ...
    add_episode_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_EPISODE_NUM: [
                _done_ep_cb,
                MessageHandler(filters.TEXT & ~filters.COMMAND, got_episode_num),
            ],
            WAIT_EPISODE_VIDEO: [
                _done_ep_cb,
