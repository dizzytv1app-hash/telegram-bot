...
async def _send_auto_backup(bot):
    ...
    if not BACKUP_CHANNEL_ID:
        logger.info("BACKUP_CHANNEL_ID sozlanmagan — avtomatik backup o'tkazib yuborildi.")
        return
...
async def _auto_backup_loop(bot):
    """Kuniga BACKUP_HOURS'da ko'rsatilgan soatlarda (Toshkent vaqti) backup kanalga yuboradi."""
    if not BACKUP_CHANNEL_ID or not BACKUP_HOURS:
        logger.info("Avtomatik backup o'chirilgan (BACKUP_CHANNEL_ID yoki BACKUP_HOURS bo'sh).")
        return
    tz = ZoneInfo(BACKUP_TIMEZONE_NAME)
    while True:
        now = datetime.now(tz)
