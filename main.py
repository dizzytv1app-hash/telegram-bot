...
# ==================== STATES ====================
(
    WAIT_ANIME_CODE, WAIT_ANIME_NAME, WAIT_ANIME_CHANNEL, WAIT_ANIME_GENRE,
    ...
    WAIT_CHPUB_CODE, WAIT_CHPUB_CHANNEL
) = range(24)
...
ADMIN_BUTTONS = {
    ...
    "🆕 Yangi Qismlar", "🛠 Qism Boshqarish", "📡 Anime Kanaliga Yuborish",
...
_MENU_BTN_FILTER = filters.Regex(
    ...
    r"|📢 Reklama|📺 Animelar Kanali|📤 Kanalga Yuborish|👥 Adminlar|🆕 Yangi Qismlar|🛠 Qism Boshqarish|📡 Anime Kanaliga Yuborish|⏭ Shorts.*)$"
)
...
def init_db():
    ...
    try:
        ...
        pass  # ustun allaqachon mavjud
    try:
        c.execute("ALTER TABLE animes ADD COLUMN channel_username TEXT")
    except sqlite3.OperationalError:
        pass  # ustun allaqachon mavjud — har bir anime uchun alohida kanal (@username shaklida)
...
def add_anime(code, name, year, genre, total_episodes, description, poster_file_id, poster_type="photo", total_episodes_label=None, channel_username=None):
    conn = get_db()
    c = conn.cursor()
    now = datetime.now()
    c.execute("""
        INSERT INTO animes (code, name, year, genre, total_episodes, description, poster_file_id, added_date, poster_type, total_episodes_label, added_at, channel_username)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (code, name, year, genre, total_episodes, description, poster_file_id, now.strftime("%Y-%m-%d"), poster_type, total_episodes_label, now.timestamp(), channel_username))
    conn.commit()

ALLOWED_ANIME_FIELDS = {"name", "year", "genre", "total_episodes", "description", "channel_username"}

def get_anime_channel(code):
    """Anime uchun biriktirilgan kanal usernameni qaytaradi (yo'q bo'lsa None)."""
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT channel_username FROM animes WHERE code=?", (code,))
    row = c.fetchone()
    return row[0] if row else None

def get_all_animes_channel_status():
    """Barcha animelarni [(code, name, channel_username), ...] shaklida, qo'shilgan tartibda qaytaradi."""
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT code, name, channel_username FROM animes ORDER BY id")
    return c.fetchall()
...
def _parse_channel_input(text):
    """Admin yuborgan kanal username/havolasini '@username' shakliga keltiradi.
    Qaytaradi: (ok, value) — ok=False bo'lsa, value xato xabari; value=None bo'lsa kanal o'chirilgan/o'tkazib yuborilgan."""
    text = text.strip()
    if text in ("-", "yo'q", "yoq", "skip"):
        return True, None
    text = text.replace("https://t.me/", "").replace("http://t.me/", "").replace("t.me/", "")
    text = text.lstrip("@").strip()
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]{4,31}", text):
        return False, None
    return True, "@" + text
...
def admin_menu_keyboard(user_id=None):
    keyboard = [
        ...
        [KeyboardButton("📡 Anime Kanaliga Yuborish")],
...
async def post_anime_to_channel(context, code, name, genre, total_episodes, poster_id, poster_type="photo", ep_label=None, target_channel=None):
    """Admin '📤 Kanalga Yuborish' tugmasi orqali bossagina, ushbu animening o'z kanaliga
    (agar biriktirilgan bo'lsa) yoki umumiy kanalga poster + ma'lumot joylaydi."""
    used_channel = target_channel or CHANNEL_USERNAME
    watch_url = f"https://t.me/{BOT_USERNAME}?start={code}"
    ep_count_display = _episode_label(total_episodes, ep_label)
    caption = (
        f"{name}\n"
        f"{name} [{ep_count_display}-qism]\n"
        f"{used_channel}"
    )
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url)]])
    try:
        if poster_type == "video":
            await context.bot.send_video(
                chat_id=used_channel, video=poster_id,
                caption=caption, reply_markup=kb
            )
        else:
            await context.bot.send_photo(
                chat_id=used_channel, photo=poster_id,
                caption=caption, reply_markup=kb
            )
        return True
...
async def handle_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text.isdigit():
        if is_admin(update.effective_user.id):
            await update.message.reply_text(
                "⚠️ Bu buyruqni tushunmadim.\n"
                "Agar biror jarayon o'rtasida qolib ketgan bo'lsangiz, kerakli tugmani "
                "qaytadan bosing yoki /cancel yuboring.",
...
async def got_anime_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "6️⃣ Ushbu anime uchun *kanal* username yoki havolasini yuboring:\n"
        "_(masalan: @mychannel yoki https://t.me/mychannel)_\n\n"
        "⚠️ Botni o'sha kanalga *admin* qilib qo'shishni unutmang, "
        "aks holda kanalga post yuborilmaydi.\n\n"
        "Agar hozircha kanal bo'lmasa, «-» yuboring _(keyin ✏️ Anime Tahrirlash orqali qo'shishingiz mumkin)_:",
        parse_mode="Markdown", reply_markup=_cancel_kb()
    )
    return WAIT_ANIME_CHANNEL

async def got_anime_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    ok, channel_username = _parse_channel_input(txt)
    if not ok:
        await update.message.reply_text(
            "⚠️ Noto'g'ri format. Kanal username yoki havolasini yuboring "
            "(masalan: @mychannel), yoki o'tkazib yuborish uchun «-» yuboring:",
            reply_markup=_cancel_kb()
        )
        return WAIT_ANIME_CHANNEL
    ...
    poster_id = d["new_anime_poster_id"]
    poster_type = d["new_anime_poster_type"]
    try:
        add_anime(
        ...
        channel_line = f"📡 Kanal: {channel_username}" if channel_username else "📡 Kanal: biriktirilmagan"
        await update.message.reply_text(
...
async def got_episode_channel_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if poster_type == "video":
            await context.bot.send_video(
                chat_id=target_channel, video=poster_id,
                caption=caption, reply_markup=kb
            )
        else:
            await context.bot.send_photo(
                chat_id=target_channel, photo=poster_id,
                caption=caption, reply_markup=kb
            )
        await update.message.reply_text(
            f"✅ {target_channel} kanaliga yuborildi: *{_esc_md(name)}* — {count} ta yangi qism.",
            parse_mode="Markdown"
        )
    except Exception as e:
...
# -- ANIME KANALIGA YUBORISH (bitta anime — o'z kanaliga to'liq joylash) --
async def channel_publish_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    context.user_data.clear()
    animes = get_all_animes_channel_status()
    if not animes:
        await update.message.reply_text("❌ Bazada hali anime yo'q.")
        return ConversationHandler.END
    header = (
        "📡 *Anime Kanaliga Yuborish*\n"
        "_(✅ — kanal biriktirilgan, ❌ — biriktirilmagan)_\n\n"
    )
    chunk = header
    for code, name, channel in animes:
        mark = "✅" if channel else "❌"
        line = f"{mark} *{code}* — {_esc_md(name)}\n"
        if len(chunk) + len(line) > 3500:
            await update.message.reply_text(chunk, parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
            chunk = ""
        chunk += line
    chunk += "\nKanalga to'liq joylamoqchi bo'lgan anime kodini yozing:"
    await update.message.reply_text(chunk, parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    ...
    return WAIT_CHPUB_CODE

async def got_chpub_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    ...
    if not txt.isdigit():
        await update.message.reply_text("⚠️ Faqat *raqam* (kod) yuboring:", parse_mode="Markdown", reply_markup=cancel_kb)
        return WAIT_CHPUB_CODE
    code = int(txt)
    anime = get_anime_by_code(code)
    if not anime:
        await update.message.reply_text("⚠️ Bunday kodli anime topilmadi. Qaytadan yuboring:", reply_markup=cancel_kb)
        return WAIT_CHPUB_CODE
    name = anime[2]
    channel = get_anime_channel(code)
    context.user_data["chpub_code"] = code
    if not channel:
        await update.message.reply_text(
            f"🎬 *{_esc_md(name)}* uchun hali kanal biriktirilmagan.\n\n"
            f"📡 Kanal username yoki havolasini yuboring:\n"
            f"_(masalan: @mychannel yoki https://t.me/mychannel)_",
            parse_mode="Markdown", reply_markup=cancel_kb
        )
        return WAIT_CHPUB_CHANNEL

    context.user_data.clear()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Yuborish", callback_data=f"chpub_send_{code}")],
        [InlineKeyboardButton("🔄 Kanalni o'zgartirish", callback_data=f"chpub_change_{code}")],
    ])
    await update.message.reply_text(
        f"🎬 *{_esc_md(name)}*\n📡 Kanal: {_esc_md(channel)}\n\n"
        f"Botni ushbu kanalga *admin* qilib qo'shganingizga ishonch hosil qiling, "
        f"so'ng «📤 Yuborish» tugmasini bosing:",
        parse_mode="Markdown", reply_markup=kb
    )
    return ConversationHandler.END

async def chpub_change_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return ConversationHandler.END
    code = int(query.data[len("chpub_change_"):])
    anime = get_anime_by_code(code)
    if not anime:
        await query.message.reply_text("❌ Bunday anime topilmadi!")
        return ConversationHandler.END
    context.user_data.clear()
    context.user_data["chpub_code"] = code
    cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")]])
    await query.message.reply_text(
        f"🎬 *{_esc_md(anime[2])}* uchun yangi kanal username yoki havolasini yuboring:\n"
        f"_(masalan: @mychannel yoki https://t.me/mychannel)_",
        parse_mode="Markdown", reply_markup=cancel_kb
    )
    return WAIT_CHPUB_CHANNEL

async def got_chpub_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")]])
    ok, channel_username = _parse_channel_input(txt)
    if not ok or channel_username is None:
        await update.message.reply_text(
            "⚠️ Noto'g'ri format. Kanal username yoki havolasini yuboring "
            "(masalan: @mychannel):",
    ...
    code = context.user_data.get("chpub_code")
    anime = get_anime_by_code(code)
    name = anime[2] if anime else str(code)
    update_anime_field(code, "channel_username", channel_username)
    context.user_data.clear()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Yuborish", callback_data=f"chpub_send_{code}")],
        [InlineKeyboardButton("🔄 Kanalni o'zgartirish", callback_data=f"chpub_change_{code}")],
    ...
    await update.message.reply_text(
        f"✅ Kanal saqlandi: {_esc_md(channel_username)}\n\n"
        f"⚠️ Botni ushbu kanalga *admin* qilib qo'shishni unutmang, aks holda yuborilmaydi.\n\n"
        f"Bot admin qilib qo'shilgandan so'ng «📤 Yuborish» tugmasini bosing:",
        parse_mode="Markdown", reply_markup=kb
    )
    return ConversationHandler.END

async def _send_video_with_retry(context, chat_id, video, caption, reply_markup, max_retries=3):
    """Flood-control (RetryAfter) va vaqtinchalik tarmoq xatolarida qayta urinadi —
    shu orqali 'ba'zi qismlar sababsiz yuborilmay qolish' muammosining oldini oladi."""
    for attempt in range(max_retries):
        try:
            await context.bot.send_video(chat_id=chat_id, video=video, caption=caption, reply_markup=reply_markup)
            return True
        except RetryAfter as e:
            await asyncio.sleep(e.retry_after + 1)
        except TelegramError as e:
            logger.warning(f"send_video xato (urinish {attempt + 1}/{max_retries}): {e}")
            await asyncio.sleep(2)
    return False

async def _show_chpub_episode_list(message, context, code):
    anime = get_anime_by_code(code)
    name = anime[2]
    seasons = get_seasons(code)
    rows, row = [], []
    has_any = False
    for sid, snum, s_poster, s_total, s_ptype, s_label in seasons:
        for ep_num in get_episodes_list(sid):
            has_any = True
            row.append(InlineKeyboardButton(str(ep_num), callback_data=f"chpub_ep_{sid}_{ep_num}"))
            if len(row) == 6:
                rows.append(row)
                row = []
    if row:
        rows.append(row)
    if not has_any:
        await message.reply_text(f"ℹ️ *{_esc_md(name)}* uchun hali yuklangan qism yo'q.", parse_mode="Markdown")
        return
    rows.append([InlineKeyboardButton("📤 Hammasini yuborish", callback_data=f"chpub_all_{code}")])
    await message.reply_text(
        f"🎬 *{_esc_md(name)}*\n\n"
        f"Qaysi qismni kanalga yubormoqchisiz? Raqamni bosing, "
        f"yoki pastdagi tugma orqali hammasini birdaniga yuboring:",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(rows)
    )

async def channel_publish_send_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("❌ Ruxsat yo'q!", show_alert=True)
        return
    code = int(query.data[len("chpub_send_"):])
    anime = get_anime_by_code(code)
    if not anime:
    ...
    channel = get_anime_channel(code)
    if not channel:
        await query.answer("❌ Bu anime uchun kanal biriktirilmagan!", show_alert=True)
        return
    await query.answer("⏳ Asosiy poster yuborilmoqda...")
    name, genre, total_ep = anime[2], anime[4], anime[5]
    poster_id = anime[7]
    _conn = get_db(); _c = _conn.cursor()
    _c.execute("SELECT poster_type, total_episodes_label FROM animes WHERE code=?", (code,))
    _row = _c.fetchone()
    poster_type = (_row[0] if _row and _row[0] else "photo")
    ep_label = (_row[1] if _row else None)

    ok = await post_anime_to_channel(
        context, code, name, genre, total_ep, poster_id,
        poster_type=poster_type, ep_label=ep_label, target_channel=channel
    )
    if not ok:
        await query.message.reply_text(
            f"❌ *{_esc_md(name)}* — {_esc_md(channel)} kanaliga yuborib bo'lmadi.\n"
            f"Botni o'sha kanalga admin qilib qo'shganingizni tekshiring.",
            parse_mode="Markdown"
        )
        return
    increment_channel_post_count(code)
    await _show_chpub_episode_list(query.message, context, code)

async def chpub_send_episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("❌ Ruxsat yo'q!", show_alert=True)
        return
    sid_str, ep_str = query.data[len("chpub_ep_"):].split("_")
    sid, ep_num = int(sid_str), int(ep_str)
    season = get_season_by_id(sid)
    if not season:
        await query.answer("❌ Fasl topilmadi!", show_alert=True)
        return
    code = season[1]
    anime = get_anime_by_code(code)
    channel = get_anime_channel(code)
    if not anime or not channel:
        ...
        return
    ep_row = get_episode(sid, ep_num)
    if not ep_row or not ep_row[0]:
        await query.answer("❌ Bu qism videosi topilmadi!", show_alert=True)
        return
    await query.answer(f"⏳ {ep_num}-qism yuborilmoqda...")
    name = anime[2]
    watch_url = f"https://t.me/{BOT_USERNAME}?start={code}"
    caption = f"{name}\n{name} [{ep_num:02d}-qism]\n{channel}"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url)]])
    ok = await _send_video_with_retry(context, channel, ep_row[0], caption, kb)
    if ok:
        await query.message.reply_text(f"✅ {ep_num}-qism {_esc_md(channel)} kanaliga yuborildi.", parse_mode="Markdown")
    else:
        await query.message.reply_text(f"❌ {ep_num}-qism yuborilmadi (kanal/bot huquqini tekshiring).")

async def chpub_send_all_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("❌ Ruxsat yo'q!", show_alert=True)
        return
    code = int(query.data[len("chpub_all_"):])
    anime = get_anime_by_code(code)
    channel = get_anime_channel(code)
    if not anime or not channel:
        await query.answer("❌ Kanal topilmadi!", show_alert=True)
        return
    await query.answer("⏳ Barcha qismlar yuborilmoqda, biroz vaqt olishi mumkin...")
    name = anime[2]
    watch_url = f"https://t.me/{BOT_USERNAME}?start={code}"
    seasons = get_seasons(code)
    sent_count = 0
    ...
    summary = f"✅ *{_esc_md(name)}* — {sent_count} ta qism {_esc_md(channel)} kanaliga yuborildi!"
    if failed_eps:
        eps_txt = ", ".join(str(n) for n in failed_eps)
        summary += f"\n⚠️ Yuborilmagan qismlar: {eps_txt}\n_(bu qismlarni yuqoridagi ro'yxatdan alohida qayta yuborishingiz mumkin)_"
...
def _edit_fields_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📝 Tavsif", callback_data="editfield_desc"),
         InlineKeyboardButton("📡 Kanal", callback_data="editfield_channel")],
        [InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")],
    ])
...
EDIT_FIELD_MAP = {
    ...
    "editfield_channel":  ("channel_username",
                            "📡 Ushbu anime uchun kanal username yoki havolasini yozing:\n"
                            "_(masalan: @mychannel yoki https://t.me/mychannel)_\n"
                            "Botni o'sha kanalga admin qilib qo'shishni unutmang.\n"
                            "Kanalni o'chirish uchun «-» yuboring."),
}
...
async def got_edit_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📡 Kanal: {_esc_md(get_anime_channel(int(txt))) if get_anime_channel(int(txt)) else 'biriktirilmagan'}\n\n"
...
async def got_edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "year":
        ...
        if not ok:
            await update.message.reply_text(
                "⚠️ Noto'g'ri format. Kanal username yoki havolasini yuboring "
                "(masalan: @mychannel), yoki o'chirish uchun «-» yuboring:",
                reply_markup=cancel_kb)
            return WAIT_EDIT_VALUE
    ...
    context.user_data.clear()
    value_display = value if value is not None else "biriktirilmagan"
    await update.message.reply_text(
        f"✅ *Muvaffaqiyatli yangilandi!*\n\nKod: *{code}* — *{field}* → `{value_display}`",
        parse_mode="Markdown",
        reply_markup=admin_menu_keyboard(update.effective_user.id)
    )
    return ConversationHandler.END
...
async def got_channel_send_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target_channel = CHANNEL_USERNAME  # 📤 Kanalga Yuborish — doim asosiy kanalga
    seasons = get_seasons(code)
    if not seasons:
    ...
    if len(seasons) == 1:
        sid, snum, poster_id, total_ep, poster_type, ep_label = seasons[0]
        ok = await post_anime_to_channel(context, code, name, genre, total_ep, poster_id, poster_type=poster_type, ep_label=ep_label, target_channel=target_channel)
        if ok:
            increment_channel_post_count(code)
            count = get_channel_post_count(code)
            used_channel = target_channel or CHANNEL_USERNAME
            await update.message.reply_text(
                f"✅ *{_esc_md(name)}* — {snum}-fasl {_esc_md(used_channel)} kanaliga yuborildi!\n"
...
async def _do_channel_send(query, context, code, name, genre, season, target_channel=None):
    ...
    if ok:
        ...
        used_channel = target_channel or CHANNEL_USERNAME
        await query.message.reply_text(
            f"✅ *{_esc_md(name)}* — {season_num}-fasl {_esc_md(used_channel)} kanaliga yuborildi!\n"
            f"📊 Bu anime posteri kanalga jami *{count}-marta* joylandi.",
            parse_mode="Markdown"
        )
    else:
...
async def chsel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not seasons:
        ...
        return
    if len(seasons) == 1:
        await _do_channel_send(query, context, code, name, genre, seasons[0], target_channel=target_channel)
        return
    await query.answer()
    buttons = [
        [InlineKeyboardButton(f"{_season_label(snum, total_ep)}", callback_data=f"chsend_{sid}")]
...
async def chsend_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name, genre = anime[2], anime[4]
    target_channel = CHANNEL_USERNAME  # 📤 Kanalga Yuborish — doim asosiy kanalga
    await _do_channel_send(query, context, code, name, genre, (season_id, season_num, poster_id, total_ep, poster_type, ep_label), target_channel=target_channel)
...
def main():
    ...
    add_anime_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_ANIME_CHANNEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_anime_channel)],
        },
    ...
    edit_anime_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_EDIT_FIELD: [CallbackQueryHandler(got_edit_field_callback, pattern="^(editfield_|editseason_|cancel_anime)")],
            WAIT_EDIT_VALUE: [
    ...
    # Anime kanaliga to'liq yuborish conversation
    channel_publish_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^📡 Anime Kanaliga Yuborish$"), channel_publish_start),
            CallbackQueryHandler(chpub_change_start, pattern="^chpub_change_\\d+$"),
        ],
        states={
            WAIT_CHPUB_CODE: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_chpub_code)],
            WAIT_CHPUB_CHANNEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_chpub_channel)],
        },
        fallbacks=_esc + [CallbackQueryHandler(cancel_anime_callback, pattern="^cancel_anime$")],
        per_message=False,
    )
    ...
    app.add_handler(channel_publish_conv)
    ...
    app.add_handler(CallbackQueryHandler(channel_publish_send_callback, pattern="^chpub_send_"))
    app.add_handler(CallbackQueryHandler(chpub_send_episode_callback, pattern="^chpub_ep_"))
    app.add_handler(CallbackQueryHandler(chpub_send_all_callback, pattern="^chpub_all_"))
    ...
    app.add_handler(MessageHandler(filters.ALL, got_channel_forward), group=-1)
    ...
    async def global_error_handler(update, context):
        """Har qanday kutilmagan xatolikni loglaydi va, iloji bo'lsa, foydalanuvchiga
        'jim' qolib ketmasligi uchun qisqa xabar yuboradi."""
        logger.error("Kutilmagan xatolik:", exc_info=context.error)
        try:
            if isinstance(update, Update) and update.effective_chat:
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="⚠️ Kutilmagan xatolik yuz berdi. Iltimos, /cancel yuboring va qaytadan urinib ko'ring."
                )
        except Exception:
            pass
