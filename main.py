...
async def auto_approve_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Majburiy obuna ro'yxatidagi (yopiq) kanallarga yuborilgan qo'shilish so'rovlarini
    admin qo'lda tasdiqlashi shart bo'lmasin deb, bot avtomatik tasdiqlaydi."""
    req = update.chat_join_request
    chat_id_str = str(req.chat.id)
    chat_username = f"@{req.chat.username}" if req.chat.username else None
    known_ids = {identifier for identifier, link, title, expires_at in get_required_channels()}
    if chat_id_str in known_ids or (chat_username and chat_username in known_ids):
        try:
            await req.approve()
        except Exception as e:
            logger.warning(f"Qo'shilish so'rovini tasdiqlashda xato: {e}")
