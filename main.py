...
async def got_channel_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Anime-Kanal Bog'lash uchun kutilayotgan forward
    if is_admin(update.effective_user.id) and context.user_data.get("awaiting_anime_channel_forward"):
        ...
        if fwd_chat is None or fwd_chat.type != "channel":
            ...
            return
        context.user_data["awaiting_anime_channel_forward"] = False
        link = context.user_data.pop("anime_channel_pending_link", None)
        code = context.user_data.get("anime_channel_code")
        anime = get_anime_by_code(code)
        if not anime:
            await msg.reply_text("❌ Xatolik: anime topilmadi, qaytadan boshlang.")
            return
        set_anime_channel(code, str(fwd_chat.id), link)
        await show_anime_channel_menu(msg, code, anime[2], link, 0)
        return
...
# -- ANIME-KANAL BOG'LASH (har bir anime uchun alohida shaxsiy kanal) --

async def anime_channel_link_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    animes = get_animes_with_channel_status()
    if not animes:
        await update.message.reply_text("❌ Hali anime yo'q!")
        return
    header = "🔗 *Anime-Kanal Bog'lash*\n_(✅ — kanalga yuborilgan, ❌ — hali yuborilmagan)_\n\n"
    chunk = header
    for code, name, post_count in animes:
        mark = "✅" if post_count else "❌"
        line = f"{mark} *{code}* — {_esc_md(name)}\n"
        if len(chunk) + len(line) > 3500:
            await update.message.reply_text(chunk, parse_mode="Markdown")
            chunk = ""
        chunk += line
    chunk += "\nQaysi anime kodini tanlaysiz?"
    await update.message.reply_text(chunk, parse_mode="Markdown")
    context.user_data["awaiting_anime_channel_code"] = True

async def got_anime_channel_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not context.user_data.get("awaiting_anime_channel_code"):
        return
    context.user_data["awaiting_anime_channel_code"] = False
    text = update.message.text.strip()
    if not text.isdigit():
        await update.message.reply_text("❌ Faqat raqam (kod) yuboring!")
        context.user_data["awaiting_anime_channel_code"] = True
    ...
    if not anime:
        await update.message.reply_text("❌ Bunday anime topilmadi!")
        return
    context.user_data["anime_channel_code"] = code
    ch = get_anime_channel(code)
    channel_id, channel_link, post_count = ch if ch and ch[0] else (None, None, 0)
    if channel_id:
        await show_anime_channel_menu(update.message, code, anime[2], channel_link, post_count)
    else:
        await update.message.reply_text(
            "📡 Bu anime uchun hali kanal bog'lanmagan.\n\nKanal havolasini yuboring:\n"
            "🔓 Ochiq kanal: https://t.me/kanal_nomi (yoki @kanal_nomi)\n"
            "🔒 Yopiq kanal: https://t.me/+XXXXXXXX"
        )
        context.user_data["awaiting_anime_channel_link"] = True

async def got_anime_channel_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    if not context.user_data.get("awaiting_anime_channel_link"):
        return
    context.user_data["awaiting_anime_channel_link"] = False
    text = update.message.text.strip()
    parsed = parse_channel_link(text)
    if not parsed:
        await update.message.reply_text(
    ...
    anime = get_anime_by_code(code)
    if not anime:
        await update.message.reply_text("❌ Xatolik: anime topilmadi, qaytadan boshlang.")
        return
    if kind == "public":
        try:
            chat = await context.bot.get_chat(f"@{value}")
            ...
            return
        link = f"https://t.me/{value}"
        set_anime_channel(code, f"@{value}", link)
        await show_anime_channel_menu(update.message, code, anime[2], link, 0)
    else:
        context.user_data["anime_channel_pending_link"] = value
        context.user_data["awaiting_anime_channel_forward"] = True
        await update.message.reply_text(
            "🔒 Bu — yopiq kanal havolasi. Kanalni avtomatik aniqlash uchun:\n\n"
            "1️⃣ Botni shu kanalga *administrator* qilib qo'shing.\n"
            "2️⃣ Keyin o'sha kanaldan istalgan xabarni shu botga *forward* (uzatib) yuboring.",
...
async def show_anime_channel_menu(message, code, name, channel_link, post_count):
    status = f"✅ Kanalga *{post_count}* marta yuborilgan" if post_count else "❌ Hali kanalga yuborilmagan"
    text = (
        f"🔗 *{_esc_md(name)}*\n"
        f"📡 Bog'langan kanal: {channel_link}\n"
        f"{status}\n\n"
        f"Qaysi qismni yubormoqchisiz? Yoki hammasini birdan yuboring."
    )
    await message.reply_text(text, parse_mode="Markdown", reply_markup=_anime_channel_episode_kb(code))

def _anime_channel_episode_kb(code, season_idx=0, page=0):
    seasons = get_seasons(code)
    buttons = []
    if not seasons:
        buttons.append([InlineKeyboardButton("📤 Hammasini Yuborish", callback_data=f"achall_{code}")])
        return InlineKeyboardMarkup(buttons)

    season_idx = max(0, min(season_idx, len(seasons) - 1))
    sid, snum, poster, total_ep, ptype, ep_label = seasons[season_idx]
    episodes = get_episodes_list(sid)
    per_page = 24
    start = page * per_page
    ...
    row = []
    for ep in page_eps:
        row.append(InlineKeyboardButton(str(ep), callback_data=f"ach1_{code}_{sid}_{ep}"))
        if len(row) == 6:
            buttons.append(row)
            row = []
    if row:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⏮", callback_data=f"achpage_{code}_{season_idx}_{page-1}"))
    ...
    if len(seasons) > 1:
        ...
        for idx, s in enumerate(seasons):
            marker = "🔵" if idx == season_idx else "⚪"
            srow.append(InlineKeyboardButton(f"{marker}{s[1]}-fasl", callback_data=f"achseason_{code}_{idx}"))
        buttons.append(srow)

    buttons.append([InlineKeyboardButton("📤 Hammasini Yuborish", callback_data=f"achall_{code}")])
    return InlineKeyboardMarkup(buttons)

async def anime_channel_episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
    ...
    if data.startswith("achpage_"):
        _, code, season_idx, page = data.split("_")
        await query.edit_message_reply_markup(
            reply_markup=_anime_channel_episode_kb(int(code), int(season_idx), int(page))
        )
        return

    if data.startswith("achseason_"):
        _, code, season_idx = data.split("_")
        await query.edit_message_reply_markup(
            reply_markup=_anime_channel_episode_kb(int(code), int(season_idx), 0)
        )
        return

    if data.startswith("ach1_"):
        _, code, sid, ep_num = data.split("_")
        code, sid, ep_num = int(code), int(sid), int(ep_num)
        ch = get_anime_channel(code)
        if not ch or not ch[0]:
            await query.message.reply_text("❌ Bu anime uchun kanal bog'lanmagan!")
            return
        channel_id = ch[0]
        chat_ref = int(channel_id) if channel_id.lstrip("-").isdigit() else channel_id
        anime = get_anime_by_code(code)
        season = get_season_by_id(sid)
        episode = get_episode(sid, ep_num)
        if not episode:
            await query.message.reply_text("❌ Bu qism hali yuklanmagan!")
            return
        seasons = get_seasons(code)
        channel_tag = f"\n{channel_id}" if channel_id.startswith("@") else ""
        try:
            watch_url = f"https://t.me/{BOT_USERNAME}?start={code}_{season[2]}_{ep_num}"
            await context.bot.send_video(
                chat_id=chat_ref,
                video=episode[0],
                caption=f"{anime[2]} [{ep_num:02d}-qism]{channel_tag}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("• Yuklab Olish •", url=watch_url)]])
            )
            increment_own_channel_post_count(code)
            await query.message.reply_text(f"✅ {ep_num}-qism kanalga yuborildi!")
        except Exception as e:
            await query.message.reply_text(f"❌ Yuborilmadi: {e}")
        return

    if data.startswith("achall_"):
        ...
        chat_ref = int(channel_id) if channel_id.lstrip("-").isdigit() else channel_id
        anime = get_anime_by_code(code)
        name, genre, total_ep, poster_id, ep_label = anime[2], anime[4], anime[5], anime[7], anime[10]
        seasons = get_seasons(code)
        await query.message.reply_text("⏳ Yuborish boshlandi, biroz kuting...")

        # 1) Asosiy poster — asosiy kanalga yuborilgan format bilan bir xil
        watch_url_main = f"https://t.me/{BOT_USERNAME}?start={code}"
        ep_count_display = _episode_label(total_ep, ep_label)
        main_caption = (
            f"🎬 {name}\n"
            f"➖➖➖➖➖➖➖➖➖➖\n\n"
            ...
            f"📢 Kanal: {channel_id}\n"
            f"➖➖➖➖➖➖➖➖➖➖\n\n"
            f"🔗 Yuklab olish: {watch_url_main}"
        )
        main_msg_id = None
        try:
            if poster_id:
                main_msg = await context.bot.send_photo(
                    chat_id=chat_ref, photo=poster_id,
                    caption=main_caption,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶️ Tomosha qilish", url=watch_url_main)]])
                )
                main_msg_id = main_msg.message_id
        except Exception as e:
            await query.message.reply_text(f"❌ Asosiy poster yuborilmadi: {e}\nDavom etilmoqda...")

        # 2) Har bir fasl, har bir qism ketma-ket — har biri ALOHIDA try/except bilan,
        #    shunda bitta qismda xato (masalan Timed out) qolgan qismlarni to'xtatib qo'ymaydi
        sent, failed = 0, 0
        channel_tag = f"\n{channel_id}" if channel_id.startswith("@") else ""
        for sid, snum, s_poster, s_total, s_ptype, s_label in seasons:
            episodes = get_episodes_list(sid)
            for ep_num in episodes:
                ...
                if not episode:
                    continue
                watch_url = f"https://t.me/{BOT_USERNAME}?start={code}_{snum}_{ep_num}"
                caption = f"{name} [{ep_num:02d}-qism]{channel_tag}"
                try:
                    await context.bot.send_video(
                        chat_id=chat_ref,
                        video=episode[0],
                    ...
                    failed += 1
                    logger.warning(f"Anime-kanal: {code}-kod, {snum}-fasl, {ep_num}-qism yuborilmadi: {e}")
                await asyncio.sleep(0.3)  # Telegram tezlik chegarasidan saqlanish uchun

        summary = f"✅ Yuborish tugadi! {sent} ta qism kanalga joylandi."
        if failed:
            summary += f"\n⚠️ {failed} ta qism xatolik tufayli o'tkazib yuborildi (masalan tarmoq/timeout) — «📤 Hammasini Yuborish»ni qayta bossangiz, faqat yetishmaganlari qayta yuboriladi."
...
async def _interrupt_fallback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    direct_map = {
        ...
        "🔗 Anime-Kanal Bog'lash": anime_channel_link_start,
    }
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(anime_channel_episode_callback, pattern="^(ach1_|achall_|achpage_|achseason_)"))
    ...
    app.add_handler(MessageHandler(filters.Regex("^🔗 Anime-Kanal Bog'lash$"), anime_channel_link_start))
