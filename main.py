...
def add_anime(code, name, year, genre, total_episodes, description, poster_file_id, poster_type="photo", total_episodes_label=None):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO animes (code, name, year, genre, total_episodes, description, poster_file_id, added_date, poster_type, total_episodes_label)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (code, name, year, genre, total_episodes, description, poster_file_id, datetime.now().strftime("%Y-%m-%d"), poster_type, total_episodes_label))
    conn.commit()
...
async def admin_recent_episodes_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_buttons = [
        ...
        for code, name, season_num, count, total_ep, ep_label in grouped
    ]
    # Telegram bitta xabarda juda ko'p tugmani qabul qilmasligi mumkin — shuning uchun bo'laklarga bo'lib yuboramiz
    CHUNK_SIZE = 30
    chunks = [all_buttons[i:i + CHUNK_SIZE] for i in range(0, len(all_buttons), CHUNK_SIZE)]
    for i, chunk in enumerate(chunks):
        header = "🆕 *So'nggi 36 soatda yangi qism qo'shilgan animelar:*\n\nKanalga xabar berish uchun animeni tanlang:" if i == 0 else f"🆕 *Davomi ({i+1}/{len(chunks)}):*"
        try:
            await update.message.reply_text(
                header,
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[b] for b in chunk])
            )
        except Exception as e:
            logger.warning(f"Yangi qismlar panelini yuborishda xato: {e}")
            await update.message.reply_text(f"⚠️ Ro'yxatning {i+1}-qismini yuborishda xatolik yuz berdi.")
...
async def channel_send_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return
    animes, _seasons_by_anime, _ep_counts = get_anime_list_summary()
    if not animes:
        await update.message.reply_text("❌ Hali anime qo'shilmagan!")
        return
    # Eng oxirgi qo'shilgan anime birinchi ko'rinishi uchun teskari tartibga solamiz
    animes = list(reversed(animes))
    all_buttons = [
    ...
    # Telegram bitta xabarda juda ko'p tugmani qabul qilmasligi mumkin — bo'laklarga bo'lib yuboramiz
    CHUNK_SIZE = 30
    chunks = [all_buttons[i:i + CHUNK_SIZE] for i in range(0, len(all_buttons), CHUNK_SIZE)]
    for i, chunk in enumerate(chunks):
        header = "📤 *Kanalga yuborish*\n\nQaysi animeni kanalga yubormoqchisiz?" if i == 0 else f"📤 *Davomi ({i+1}/{len(chunks)}):*"
        try:
            await update.message.reply_text(
                header,
                parse_mode="Markdown",
            ...
            logger.warning(f"Kanalga yuborish ro'yxatini yuborishda xato: {e}")
            await update.message.reply_text(f"⚠️ Ro'yxatning {i+1}-qismini yuborishda xatolik yuz berdi.")
