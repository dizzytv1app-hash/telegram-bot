...
(
    ...
    WAIT_PREVIEW_ANIME, WAIT_PREVIEW_IMAGE
) = range(22)
...
ADMIN_BUTTONS = {
    ...
    "🆕 Yangi Qismlar", "🛠 Qism Boshqarish", "🖼 Video Prewyu",
...
_MENU_BTN_FILTER = filters.Regex(
    ...
    r"|📢 Reklama|📺 Animelar Kanali|📤 Kanalga Yuborish|👥 Adminlar|🆕 Yangi Qismlar|🛠 Qism Boshqarish|🖼 Video Prewyu|⏭ Shorts.*)$"
)
...
def set_episode_thumbnail(code, file_id):
    conn = sqlite3.connect("anime.db")
    c = conn.cursor()
    c.execute("UPDATE animes SET episode_thumb_file_id=? WHERE code=?", (file_id, code))
    conn.commit()
    conn.close()

def get_episode_thumbnail(code):
    conn = sqlite3.connect("anime.db")
    c = conn.cursor()
    c.execute("SELECT episode_thumb_file_id FROM animes WHERE code=?", (code,))
    row = c.fetchone()
    conn.close()
...
def admin_menu_keyboard(user_id=None):
    keyboard = [
        ...
        [KeyboardButton("🛠 Qism Boshqarish"), KeyboardButton("🖼 Video Prewyu")],
    ]
    if user_id == ADMIN_ID:
...
async def episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ep_"):
        ...
        if episode:
            ...
            thumb = None
            thumb_id = get_episode_thumbnail(int(anime_code))
            if thumb_id:
                try:
                    thumb_file = await context.bot.get_file(thumb_id)
                    thumb = await thumb_file.download_as_bytearray()
                except Exception as e:
                    logger.warning(f"Video prewyu olishda xato: {e}")
...
# -- VIDEO PREWYU (barcha qismlar uchun bitta rasm) --
async def preview_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return WAIT_PREVIEW_ANIME

async def got_preview_anime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = update.message.text.strip()
    if not txt.isdigit():
        await update.message.reply_text("⚠️ Faqat raqam (kod) yuboring!")
        return WAIT_PREVIEW_ANIME
    ...
    if not anime:
        await update.message.reply_text("⚠️ Bunday anime topilmadi! Kodini qayta yuboring:")
        return WAIT_PREVIEW_ANIME
    context.user_data["preview_code"] = code
    context.user_data["preview_name"] = anime[2]
    await update.message.reply_text(
        f"✅ Anime: *{_esc_md(anime[2])}*\n\n"
        f"🖼 Endi bitta rasm yuboring — shu rasm animening *barcha qismlari* uchun video prewyu (ustidagi rasm) bo'lib qoladi:",
        parse_mode="Markdown"
    )
    return WAIT_PREVIEW_IMAGE

async def got_preview_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.photo:
        await update.message.reply_text("⚠️ Faqat rasm (foto) yuboring:")
        return WAIT_PREVIEW_IMAGE
    # Telegram thumbnail talabi: <=320x320 va <200KB. Eng katta (asl) o'lcham
    # buning uchun juda katta bo'lgani sabab, mos keladigan eng kichik/o'rta
    # o'lchamni tanlaymiz (photo ro'yxati kichikdan kattaga qarab keladi).
    suitable = [p for p in update.message.photo if p.width <= 320 and p.height <= 320]
    chosen = suitable[-1] if suitable else update.message.photo[0]
    file_id = chosen.file_id
    code = context.user_data.get("preview_code")
    name = context.user_data.get("preview_name", "")
    set_episode_thumbnail(code, file_id)
    context.user_data.clear()
    await update.message.reply_text(
        f"✅ *{_esc_md(name)}* uchun video prewyu o'rnatildi!\n\n"
        f"Endi shu animening barcha qismlari shu rasm bilan ko'rsatiladi.",
        parse_mode="Markdown",
        reply_markup=admin_menu_keyboard(update.effective_user.id)
    )
    return ConversationHandler.END
...
def main():
    ...
    # Video preview conversation
    preview_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🖼 Video Prewyu$"), preview_start)],
        states={
            WAIT_PREVIEW_ANIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_preview_anime)],
            WAIT_PREVIEW_IMAGE: [MessageHandler(filters.PHOTO, got_preview_image)],
        },
        fallbacks=_esc,
        per_message=False,
    )
