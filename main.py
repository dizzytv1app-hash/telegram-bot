...
(
    ...
    WAIT_ACHAN_ANIME,   # Anime kanaliga biriktirish — anime kodi
    WAIT_ACHAN_LINK,    # Anime kanaliga biriktirish — kanal link/username
) = range(24)
...
def init_db():
    ...
    try:
        c.execute("ALTER TABLE animes ADD COLUMN anime_channel TEXT")
    except sqlite3.OperationalError:
        pass  # ustun allaqachon mavjud
...
def get_anime_channel(code):
    """Animega biriktirilgan kanalning identifikatorini qaytaradi (yoki None)."""
    conn = get_db()
    c = conn.cursor()
    c.execute("SELECT anime_channel FROM animes WHERE code=?", (code,))
    row = c.fetchone()
    return row[0] if row and row[0] else None

def set_anime_channel(code, channel):
    """Animega kanal biriktiradi (None berilsa — o'chiradi)."""
    conn = get_db()
    c = conn.cursor()
    c.execute("UPDATE animes SET anime_channel=? WHERE code=?", (channel, code))
    conn.commit()
...
def admin_menu_keyboard(user_id=None):
    keyboard = [
        ...
        [KeyboardButton("🛠 Qism Boshqarish"), KeyboardButton("📌 Anime Kanali")],
    ]
    if user_id == ADMIN_ID:
...
async def post_anime_to_channel(context, code, name, genre, total_episodes, poster_id, poster_type="photo", ep_label=None, channel_override=None):
    """Admin '📤 Kanalga Yuborish' tugmasi orqali bossagina kanalga poster + ma'lumot joylash.
    channel_override berilsa — o'sha kanalga yuboradi, aks holda animening o'z kanalini,
    u ham yo'q bo'lsa — asosiy CHANNEL_USERNAME ga yuboradi."""
    target_channel = channel_override or get_anime_channel(code) or CHANNEL_USERNAME
...
async def got_episode_channel_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Animening o'z kanalini ishlatamiz, yo'q bo'lsa asosiy kanal
...
async def got_episode_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    anime_code = d.get("ep_anime_code")
    anime_name = d.get("ep_anime_name", "")
    add_episode(season_id, ep_num, file_id)
    d["ep_added_count"] = d.get("ep_added_count", 0) + 1
    d["ep_next_num"] = ep_num + 1

    # Animening o'z kanaliga avtomatik post yuborish
    anime_channel = get_anime_channel(anime_code) if anime_code else None
    channel_status = ""
    if anime_channel:
        watch_url = f"https://t.me/{BOT_USERNAME}?start={anime_code}"
        season_label = f"{season_num}-fasl — " if season_num else ""
        caption = (
            f"🎬 *{_esc_md(anime_name)}*\n"
            f"📺 {season_label}{ep_num}-qism\n\n"
            f"👇 Yuklab olish:"
        )
        ...
        try:
            ...
            logger.warning(f"Animening kanaliga post yuborishda xato: {e}")
            channel_status = f"\n📢 Kanal: ❌ yuborilmadi"

    await update.message.reply_text(
        f"✅ *{anime_name} — {season_num}-fasl — {ep_num}-qism* saqlandi!{channel_status}\n\n"
...
# ==================== ANIME KANALI BIRIKTIRISH ====================

async def achan_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """📌 Anime Kanali — animega alohida kanal biriktirish yoki o'chirish."""
    if not is_admin(update.effective_user.id):
        return ConversationHandler.END
    context.user_data.clear()
    animes = get_all_animes()
    if not animes:
        await update.message.reply_text("❌ Hali anime qo'shilmagan!", reply_markup=admin_menu_keyboard(update.effective_user.id))
        return ConversationHandler.END
    lines = []
    for code, name, year, genre, total_ep in animes:
        ch = get_anime_channel(code)
        ch_label = f" ✅ {ch}" if ch else " ❌ kanal yo'q"
        lines.append(f"*{code}* — {_esc_md(name)}{ch_label}")
    header = "📌 *Anime Kanali Biriktirish*\n\nAnime kodini yuboring:\n\n"
    chunk = header
    for line in lines:
    ...
    if chunk.strip():
        await update.message.reply_text(chunk, parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    await update.message.reply_text("👇", reply_markup=_cancel_kb())
    return WAIT_ACHAN_ANIME

async def got_achan_anime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    if not txt.isdigit():
        await update.message.reply_text("⚠️ Faqat raqam (kod) yuboring:", reply_markup=_cancel_kb())
        return WAIT_ACHAN_ANIME
    code = int(txt)
    anime = get_anime_by_code(code)
    if not anime:
        await update.message.reply_text("⚠️ Bunday anime topilmadi! Qaytadan yuboring:", reply_markup=_cancel_kb())
        return WAIT_ACHAN_ANIME
    context.user_data["achan_code"] = code
    context.user_data["achan_name"] = anime[2]
    existing = get_anime_channel(code)
    existing_text = f"\n\nHozirgi kanal: `{existing}`" if existing else "\n\nHozircha kanal biriktirilmagan."
    remove_btn = []
    if existing:
        remove_btn = [InlineKeyboardButton("🗑 Kanalni olib tashlash", callback_data="achan_remove")]
    buttons = []
    if remove_btn:
        buttons.append(remove_btn)
    buttons.append([InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_anime")])
    await update.message.reply_text(
        f"📌 *{_esc_md(anime[2])}* uchun kanal:\n\n"
        f"Kanal havolasini yuboring:\n"
        f"• `@kanal_nomi`\n"
        f"• `https://t.me/kanal_nomi`\n"
        f"• Yopiq kanal uchun: `https://t.me/+XXXXX`{existing_text}",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(buttons)
    )
    return WAIT_ACHAN_LINK

async def achan_remove_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    code = context.user_data.get("achan_code")
    name = context.user_data.get("achan_name", "")
    if code:
        set_anime_channel(code, None)
    context.user_data.clear()
    await query.message.reply_text(
        f"🗑 *{_esc_md(name)}* kanaldan uzildi.",
        parse_mode="Markdown",
        reply_markup=admin_menu_keyboard(update.effective_user.id)
    )
    return ConversationHandler.END

async def got_achan_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    code = context.user_data.get("achan_code")
    ...
    if not parsed:
        await update.message.reply_text(
            ...
            "• `@kanal_nomi`\n"
            "• `https://t.me/kanal_nomi`\n"
            "• `https://t.me/+XXXXXXXX` (yopiq kanal)",
            parse_mode="Markdown",
            reply_markup=_cancel_kb()
        )
        return WAIT_ACHAN_LINK
    ...
    if kind == "public":
        try:
            await context.bot.get_chat(f"@{value}")
        except Exception as e:
            await update.message.reply_text(
                f"❌ Kanal topilmadi yoki botga ruxsat yo'q.\n"
                f"Botni `@{value}` kanalga *administrator* qilib qo'shganingizga ishonch hosil qiling.\n\n"
                f"_Xato: {_esc_md(str(e))}_",
                parse_mode="Markdown",
                reply_markup=_cancel_kb()
            )
            return WAIT_ACHAN_LINK
        channel_id = f"@{value}"
        set_anime_channel(code, channel_id)
        context.user_data.clear()
        await update.message.reply_text(
            f"✅ *{_esc_md(name)}* animesiga kanal biriktirildi:\n`{channel_id}`\n\n"
            f"Endi har qism qo'shilganda shu kanalga avtomatik post yuboriladi.",
            parse_mode="Markdown",
            reply_markup=admin_menu_keyboard(update.effective_user.id)
        )
        return ConversationHandler.END
    else:
        # Yopiq kanal — forward bilan aniqlash
        context.user_data["achan_pending_link"] = value
        context.user_data["achan_awaiting_forward"] = True
        await update.message.reply_text(
            "🔒 Yopiq kanal uchun:\n\n"
            "1️⃣ Botni shu kanalga *administrator* qilib qo'shing.\n"
            "2️⃣ O'sha kanaldan istalgan xabarni shu botga *forward* qiling.",
            parse_mode="Markdown"
        )
        return WAIT_ACHAN_LINK

async def got_achan_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Yopiq kanal uchun anime-kanal biriktirish jarayonidagi forward xabarni tutadi."""
    if not update.message:
        return
    if not (is_admin(update.effective_user.id) and context.user_data.get("achan_awaiting_forward")):
        return
    msg = update.message
    fwd_chat = getattr(msg, "forward_from_chat", None)
    if fwd_chat is None:
        ...
        fwd_chat = getattr(origin, "chat", None) if origin else None
    if fwd_chat is None or fwd_chat.type != "channel":
        await msg.reply_text("❌ Bu — kanaldan forward qilingan post emas. O'sha yopiq kanaldan bitta postni forward qiling.")
        return
    context.user_data["achan_awaiting_forward"] = False
    code = context.user_data.get("achan_code")
    name = context.user_data.get("achan_name", "")
    channel_id = str(fwd_chat.id)
    set_anime_channel(code, channel_id)
    context.user_data.clear()
    await msg.reply_text(
        f"✅ *{_esc_md(name)}* animesiga yopiq kanal biriktirildi:\n`{fwd_chat.title}` (`{channel_id}`)\n\n"
        f"Endi har qism qo'shilganda shu kanalga avtomatik post yuboriladi.",
        parse_mode="Markdown",
        reply_markup=admin_menu_keyboard(update.effective_user.id)
    )
...
def main():
    ...
    # Anime kanali biriktirish conversation
    achan_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^📌 Anime Kanali$"), achan_start)],
        states={
            WAIT_ACHAN_ANIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_achan_anime)],
            WAIT_ACHAN_LINK: [
                CallbackQueryHandler(achan_remove_callback, pattern="^achan_remove$"),
                CallbackQueryHandler(cancel_anime_callback, pattern="^cancel_anime$"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, got_achan_link),
            ],
        },
        fallbacks=_esc + [CallbackQueryHandler(cancel_anime_callback, pattern="^cancel_anime$")],
        per_message=False,
    ...
    app.add_handler(MessageHandler(filters.ALL, got_achan_forward), group=-2)
