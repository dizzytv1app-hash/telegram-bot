...
# Saytdagi adminlik paneli manzili — /panel buyrug'i shu yerga olib boradi
# (Telegram Web App sifatida, brauzerga chiqmasdan ochiladi).
WEBAPP_URL = os.environ.get("WEBAPP_URL", "https://animelar-api-production-2e98.up.railway.app/?admin=1")

# Saytning ochiq (admin bo'lmagan) manzili — "Shorts" bosilganda shu yerga
# havola beriladi.
SITE_URL = os.environ.get("SITE_URL", "https://animelar-api-production-2e98.up.railway.app/")

# Avtomatik zaxira nusxa har 2 kunda shu vaqtda (Toshkent bo'yicha) yuboriladi
...
(
    ...
    WAIT_ANIME_ORIGINAL, WAIT_ANIME_RATING, WAIT_START_VIDEO,
) = range(26)
...
ADMIN_BUTTONS = {
    ...
    "🔍 Anime Izlash", "⏭ Shorts", "📢 Reklama", "📺 Animelar Kanali"
}
...
def log_error(error_text, update_summary=""):
    conn = get_db()
    c = conn.cursor()
    c.execute(
        "INSERT INTO error_log (created_at, error_text, update_summary) VALUES (?, ?, ?)",
        (datetime.now(TASHKENT_TZ).isoformat(), error_text[:4000], update_summary[:500])
    )
    conn.commit()
    # Jadval cheksiz o'smasligi uchun, faqat oxirgi 500 tasini saqlaymiz
    c.execute(
...
# Eslatma: matn ichidagi "custom emoji" (qimirlovchi belgi) Telegram tomonidan
# faqat Premium yoki Fragment username'ga ega botlarga ruxsat etiladi — bepul
# emas. Shu sababli bu yerda ochiq, bepul animatsion stiker to'plamidan
# foydalanamiz (@animatedemojies) — bu har qanday bot va foydalanuvchi uchun,
# hech qanday Premium'siz ishlaydi.
STICKER_SET_NAME = "AnimatedEmojies"

# Voqea nomi -> shu to'plamdagi stikerni tanlaydigan emoji
STICKER_EVENTS = {
    "welcome": "👋",
    "success": "✅",
    "party": "🎉",
...
_sticker_cache = {}  # emoji -> file_id (bir marta so'ralib, keyin keshdan olinadi)

async def _get_sticker_file_id(bot, emoji: str):
    """Berilgan emojiga mos animatsion stiker file_id sini qaytaradi (keshlanadi)."""
    if emoji in _sticker_cache:
        return _sticker_cache[emoji]
    try:
        sticker_set = await bot.get_sticker_set(STICKER_SET_NAME)
        for sticker in sticker_set.stickers:
            if sticker.emoji == emoji:
                _sticker_cache[emoji] = sticker.file_id
                return sticker.file_id
    except Exception as e:
        logger.warning(f"Animatsion stiker olinmadi ({emoji}): {e}")
    return None

async def send_event_sticker(bot, chat_id, event: str):
    """Voqea turiga mos animatsion stiker yuboradi (masalan 'success', 'welcome').
    Stiker topilmasa yoki xatolik yuz bersa, botning asosiy ishiga xalaqit bermaydi."""
    emoji = STICKER_EVENTS.get(event)
    if not emoji:
        return
    file_id = await _get_sticker_file_id(bot, emoji)
    if not file_id:
        return
    try:
        await bot.send_sticker(chat_id=chat_id, sticker=file_id)
    except Exception as e:
        logger.warning(f"Stiker yuborilmadi: {e}")

def main_menu_keyboard():
    keyboard = [
        [KeyboardButton("🔍 Anime Izlash")],
        [KeyboardButton("⏭ Shorts"), KeyboardButton("📢 Reklama")],
...
async def send_anime_info(bot, chat_id, code):
    ...
    if not seasons:
        ...
        await _send_season_info(bot, chat_id, name, genre, desc, None, poster_id, total_ep, code=code, status_key=status_key)
        return
...
async def send_start(update, context, deep_code=None):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚙️ Kabinet", callback_data="kabinet"), InlineKeyboardButton("🌐 Sayt", callback_data="shorts")],
        [InlineKeyboardButton("📺 Animelar Kanali", url=CHANNEL_LINK), InlineKeyboardButton("📢 Reklama", callback_data="reklama")],
    ])
    start_video_id = get_setting("start_video_file_id")
    start_video_type = get_setting("start_video_type")
    if start_video_id and start_video_type == "animation":
        await update.message.reply_animation(animation=start_video_id, caption=text, reply_markup=keyboard)
    elif start_video_id:
        await update.message.reply_video(video=start_video_id, caption=text, reply_markup=keyboard)
    else:
        await update.message.reply_text(text, reply_markup=keyboard)

async def panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/panel — faqat adminlar uchun. Saytdagi tezkor adminlik panelini
    Telegram Web App sifatida ochadigan tugma yuboradi (brauzerga
    chiqmasdan, Telegram ichida). Kirish parolsiz — Telegram o'zi
    foydalanuvchini xavfsiz tasdiqlaydi."""
    user = update.effective_user
    if not is_admin(user.id):
        await update.message.reply_text("❌ Bu buyruq faqat adminlar uchun.")
        return
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌐 Panelni ochish", web_app=WebAppInfo(url=WEBAPP_URL))]
    ])
    await update.message.reply_text(
        "🛠 Saytdagi tezkor adminlik paneli:\n"
        "Bu yerda banner, statistika va animelarni tez tahrirlash imkoniyati bor.",
        reply_markup=keyboard
    )

async def set_start_video_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/setstartvideo — faqat adminlar uchun. /start bosilganda chiqadigan
    videoni o'rnatadi — admin yuborgan video shu o'rindan keyin doimiy
    saqlanadi (bazaga yoziladi), qayta deploy qilinsa ham yo'qolmaydi."""
    user = update.effective_user
    if not is_admin(user.id):
        await update.message.reply_text("❌ Bu buyruq faqat adminlar uchun.")
        return ConversationHandler.END
    await update.message.reply_text(
        "🎬 /start bosilganda chiqadigan videoni yuboring.\n\n"
        "Bekor qilish uchun /cancel yuboring."
    )
    return WAIT_START_VIDEO

async def got_start_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.animation:
        file_id = update.message.animation.file_id
        set_setting("start_video_type", "animation")
    elif update.message.video:
        file_id = update.message.video.file_id
        set_setting("start_video_type", "video")
    else:
        await update.message.reply_text("❗️ Iltimos, video yoki GIF yuboring.")
        return WAIT_START_VIDEO
    set_setting("start_video_file_id", file_id)
    await update.message.reply_text("✅ Saqlandi! Endi /start bosilganda shu chiqadi.")
    return ConversationHandler.END

async def cancel_start_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Bekor qilindi.")
    return ConversationHandler.END

async def set_backup_channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/setbackupchannel — faqat adminlar uchun. Har 2 kunda ertalab soat
    7:00da (Toshkent vaqti) avtomatik zaxira nusxa yuboriladigan kanalni
    o'rnatadi."""
    user = update.effective_user
    if not is_admin(user.id):
        await update.message.reply_text("❌ Bu buyruq faqat adminlar uchun.")
    ...
    await update.message.reply_text(
        "💾 Avtomatik zaxira nusxa yuboriladigan kanalni sozlaymiz.\n\n"
...
async def got_backup_channel_forward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not fwd_chat or fwd_chat.type != "channel":
        await update.message.reply_text("❌ Bu kanal xabari emas. Iltimos, kanaldan xabarni forward qiling.")
        return
    context.user_data["awaiting_backup_channel_forward"] = False
    try:
        await context.bot.send_chat_action(chat_id=fwd_chat.id, action="typing")
    except Exception as e:
        await update.message.reply_text(
            f"❌ Botga bu kanalda ruxsat yo'q. Botni administrator qilib qo'shganingizga ishonch hosil qiling.\n\n"
            f"_Xato: {_esc_md(str(e))}_",
            parse_mode="Markdown"
        )
        return
    set_setting("backup_channel_id", str(fwd_chat.id))
    await update.message.reply_text(
        f"✅ *{_esc_md(fwd_chat.title or str(fwd_chat.id))}* — zaxira nusxa kanali sifatida saqlandi.\n"
        f"Endi har 2 kunda ertalab soat {BACKUP_HOUR}:00da (Toshkent vaqti) shu kanalga avtomatik zaxira yuboriladi.",
        parse_mode="Markdown"
    )

async def global_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Botda qayerda bo'lmasin kutilmagan xatolik chiqsa, shu yerga tushadi:
    xatolik bazaga (error_log) yoziladi va adminga qisqacha xabar yuboriladi."""
    import traceback
    tb_text = "".join(traceback.format_exception(None, context.error, context.error.__traceback__))
    logger.error(f"Kutilmagan xatolik: {tb_text}")

    update_summary = ""
    try:
        if isinstance(update, Update) and update.effective_user:
            update_summary = f"user_id={update.effective_user.id}"
            if update.effective_message and update.effective_message.text:
                update_summary += f", text={update.effective_message.text[:200]}"
    except Exception:
        pass

    try:
        log_error(tb_text, update_summary)
    except Exception as e:
        logger.warning(f"Xatolikni bazaga yozib bo'lmadi: {e}")

    try:
        short = tb_text.strip().splitlines()[-1] if tb_text.strip() else "Noma'lum xatolik"
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"⚠️ Botda xatolik yuz berdi:\n\n`{_esc_md(short[:500])}`\n\n{_esc_md(update_summary)}",
            parse_mode="Markdown"
        )
    except Exception:
        pass

async def send_scheduled_backup(context: ContextTypes.DEFAULT_TYPE):
    """JobQueue orqali har 2 kunda ishga tushadigan vazifa — bazaning
    zaxira nusxasini backup_channel_id'da saqlangan kanalga yuboradi."""
    channel_id = get_setting("backup_channel_id")
    if not channel_id:
        return
    conn = get_db()
    timestamp = datetime.now(TASHKENT_TZ).strftime("%Y_%m_%d_%H%M%S")
    ...
    if user_is_admin:
        ...
        if start_video_id and start_video_type == "animation":
            await context.bot.send_animation(
                chat_id=user.id, animation=start_video_id, caption=text, reply_markup=keyboard
            )
        elif start_video_id:
            await context.bot.send_video(
                chat_id=user.id, video=start_video_id, caption=text, reply_markup=keyboard
            )
        else:
            await context.bot.send_message(
                chat_id=user.id,
...
async def shorts_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔴 Shorts'larni saytimizda tomosha qiling:",
        reply_markup=keyboard
    )
...
async def got_anime_status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        extra_text = ("\n".join(extra_lines) + "\n") if extra_lines else ""
        await send_event_sticker(context.bot, query.message.chat_id, "party")
        await query.message.reply_text(
...
async def done_episodes_confirm_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    count = context.user_data.get("ep_added_count", 0)
    context.user_data.clear()
    await send_event_sticker(context.bot, query.message.chat_id, "success")
...
async def got_newseason_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_event_sticker(context.bot, update.effective_chat.id, "fire")
    await update.message.reply_text(
...
async def got_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    delete_anime(code)
    context.user_data.clear()
    await send_event_sticker(context.bot, query.message.chat_id, "delete")
...
def main():
    ...
    # /setstartvideo conversation — /start bosilganda chiqadigan videoni o'rnatish
    set_start_video_conv = ConversationHandler(
        entry_points=[CommandHandler("setstartvideo", set_start_video_start)],
        states={
            WAIT_START_VIDEO: [MessageHandler(filters.VIDEO | filters.ANIMATION, got_start_video)],
        },
        fallbacks=[CommandHandler("cancel", cancel_start_video)],
        per_message=False,
    )
    ...
    app.add_handler(set_start_video_conv)
    app.add_handler(CommandHandler("setbackupchannel", set_backup_channel_command))
    app.add_handler(MessageHandler(filters.FORWARDED, got_backup_channel_forward), group=-2)
    ...
    # Har 2 kunda ertalab soat 7:00da (Toshkent) avtomatik zaxira nusxa
    if app.job_queue is not None:
        app.job_queue.run_repeating(
            send_scheduled_backup,
            interval=timedelta(days=2),
            first=dtime(hour=BACKUP_HOUR, minute=BACKUP_MINUTE, tzinfo=TASHKENT_TZ),
        )
    else:
        logger.warning(
            "JobQueue mavjud emas — avtomatik backup ishlamaydi. "
            "requirements.txt'da 'python-telegram-bot[job-queue]' bo'lishi kerak."
        )
