...
# olishini o'lchash uchun. Haqiqiy Telegram xabar yubormaydi (bu Telegramning
# o'z flood-limitini sinaydi, bizning server quvvatimizni emas) — o'rniga
# real foydalanuvchi so'rovi bosganda ishlaydigan bazadan o'qish ishini
# (get_anime_by_code + get_seasons + get_episodes_list) N marta bir vaqtda
# ishga tushiradi va har birining necha soniyada tugaganini o'lchaydi.
LOAD_TEST_MAX_N = 2000  # bitta testda ruxsat etilgan eng ko'p so'rov soni
...
async def load_test_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "_Bu test haqiqiy foydalanuvchilarga xabar yubormaydi — faqat "
        "serverning bazadan o'qish tezligini o'lchaydi._",
        parse_mode="Markdown"
    )
...
async def _load_test_one_request(code):
    """Bitta 'virtual foydalanuvchi' so'rovini simulyatsiya qiladi: real
    foydalanuvchi anime ochganda ishlaydigan bazadan o'qish zanjirini bajaradi."""
...
async def got_load_test_n(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    results = await asyncio.gather(*[_load_test_one_request(code) for _ in range(n)])
